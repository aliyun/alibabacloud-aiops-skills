...
if ! aliyun cloudfw describe-asset-list \
  --current-page 1 \
  --page-size 10 \
  --search-item "${INSTANCE_ID}" \
  --resource-type BastionHostIngressIP \
  --user-agent "${USER_AGENT}"; then
  echo "Error: failed to query Cloud Firewall asset list for Bastion Host instance ${INSTANCE_ID}. Please verify the instance ID and permissions." >&2
  exit 1
fi
