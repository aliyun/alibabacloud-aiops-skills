...
if ! aliyun yundun-bastionhost describe-instances --biz-region-id "${REGION}" --region "${REGION}" --user-agent "${USER_AGENT}"; then
  echo "Error: failed to query Bastion Host instances in region ${REGION}. Please check credentials, permissions, and network connectivity." >&2
  exit 1
fi
