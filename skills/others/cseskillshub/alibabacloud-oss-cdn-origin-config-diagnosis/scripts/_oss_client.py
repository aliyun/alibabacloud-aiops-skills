#!/usr/bin/env python3
"""
_oss_client.py -- Shared OSS control-plane client for CDN origin diagnosis
===========================================================================
...
values. Only the bucket metadata query GetBucketInfo is ever issued against
OSS; no mutating call exists anywhere in this skill.
...
Channels (measured, finalized):
  * OSS control-plane metadata -> Python oss2 SDK (>= 2.19.0, < 3). The
    aliyun CLI carries no OSS control-plane metadata, so the SDK is the only
    viable channel for the origin-bucket cross-check (ACL / owner / region).
...
def _normalize_oss_error(exc: Exception, operation: str) -> OssClientError:
    ...
    if isinstance(exc, oss2.exceptions.AccessDenied):
        hint = ("Access denied (403): the caller lacks oss:GetBucketInfo "
                "permission, the bucket belongs to another account, or the "
                "endpoint's region differs from the bucket's region. For a "
                "CDN origin bucket owned by another account this is expected "
                "-- the cross-account origin authorization check then relies "
                "on the playbook guidance instead of the live metadata.")
        return OssClientError(
