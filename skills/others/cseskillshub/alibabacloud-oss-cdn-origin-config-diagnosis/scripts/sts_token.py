...
def main() -> None:
    ...
    summary = (
        ...
        f"(identity type: {identity_type}). CDN back-to-origin diagnostics "
        f"run by this skill will cover the CDN domains and OSS buckets of "
        f"this account (or buckets this account is allowed to read)."
    )
    key_findings = [
        f"Account UID: {uid}",
    ...
    suggestions = [
        "Confirm this UID matches the account that owns the accelerated "
        "domain before running the origin diagnosis script.",
        "Proceed to origin_config_diagnosis.py with --domain (and optionally "
        "--user-origin-host / --uid).",
    ]
    ...
    if args.json:
        ...
        print("Next step       : if this is the account you expect, proceed "
              "with origin_config_diagnosis.py. If not, switch to the "
              "correct credential profile first.")
    print("\nSTATUS: OK")
    print(f"NEXT_ACTION: run origin_config_diagnosis.py with --domain "
          f"(and optionally --user-origin-host / --uid) to diagnose the CDN "
          f"back-to-origin configuration")
