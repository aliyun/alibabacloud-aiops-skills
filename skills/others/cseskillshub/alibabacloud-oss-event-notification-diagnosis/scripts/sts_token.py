...
def main() -> None:
    ...
    summary = (
        ...
        f"(identity type: {identity_type}). OSS event-notification and "
        f"upload-callback diagnostics run by this skill will cover this "
        f"account only."
    )
    key_findings = [
        f"Account UID: {uid}",
    ...
    suggestions = [
        ...
        "before running the event-notification diagnosis script.",
        "Proceed to oss_event_notification_diagnosis.py with --bucket (and "
        "optionally --region / --expected-event / --callback-error).",
    ]
    ...
    if args.json:
        ...
        print("Next step       : if this is the account you expect, proceed "
              "with oss_event_notification_diagnosis.py. If not, switch to "
              "the correct credential profile first.")
    print("\nSTATUS: OK")
    print(f"NEXT_ACTION: run oss_event_notification_diagnosis.py with "
          f"--bucket (and optionally --region / --expected-event / "
          f"--callback-error) to diagnose event notification or upload "
          f"callback issues")
