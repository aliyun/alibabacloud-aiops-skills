...
def main() -> None:
    ...
    summary = (
        ...
        f"(identity type: {identity_type}). Direct-access-link diagnostics "
        f"run by this skill will cover this account only."
    )
    key_findings = [
        f"Account UID: {uid}",
    ...
    suggestions = [
        ...
        "before running the direct-access-link diagnosis script.",
        "Proceed to oss_direct_access_diagnosis.py with --bucket (and "
        "optionally --object / --domain / --region).",
    ]
    ...
    if args.json:
        ...
        print("Next step       : if this is the account you expect, proceed "
              "with oss_direct_access_diagnosis.py. If not, switch to the "
              "correct credential profile first.")
    print("\nSTATUS: OK")
    print(f"NEXT_ACTION: run oss_direct_access_diagnosis.py with --bucket "
          f"(and optionally --object / --domain / --region) to diagnose the "
          f"direct-access link (preview-vs-download, custom domain binding, "
          f"referer hotlink protection)")
