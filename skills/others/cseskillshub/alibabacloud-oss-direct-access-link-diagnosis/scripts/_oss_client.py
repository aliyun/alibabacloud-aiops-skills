#!/usr/bin/env python3
"""
_oss_client.py -- Shared OSS control-plane client for direct-access-link diagnosis
===================================================================================
...
values. Only the metadata queries GetBucketInfo, GetBucketReferer,
GetObjectAcl, GetBucketWebsite, ListBucketCname and ListBuckets are ever
issued against OSS; no mutating call exists anywhere in this skill (PutBucketReferer /
PutBucketWebsite and any other write API are ABSOLUTELY PROHIBITED --
...
Measured behavior (oss2 2.19.1 against real buckets):
  * get_bucket_referer() always returns a configuration: a bucket without
    an explicit referer rule reports allow_empty_referer=True and
    referers=[] (the OSS default). This is the semantic state
    "hotlink protection not configured", NOT an error.
  * get_bucket_website() raises a 404 OssError with
    Code=NoSuchWebsiteConfiguration when static website hosting is not
    configured. It is mapped to {"configured": False} (a configuration
    state, not an operational error).
  * list_bucket_cname() returns cnames=[] when no custom domain is bound.
  * Non-existent bucket: NoSuchBucket (404) as usual.
  * Missing permission: AccessDenied (403), normalized to
...
_SKILL_NAME = "alibabacloud-oss-direct-access-link-diagnosis"
...
def _normalize_oss_error(exc: Exception, operation: str) -> OssClientError:
    ...
    if isinstance(exc, oss2.exceptions.AccessDenied):
        ...
        hint = ("Check RAM permission (oss:GetBucketInfo / "
                ...
                "oss:ListBucketCname / oss:GetObjectAcl for this caller, "
                "see references/ram-policies.md), or confirm the bucket "
...
def get_bucket_referer(bucket_name: str, endpoint: str,
                       timeout: int = _DEFAULT_TIMEOUT) -> dict:
    """Call OSS GetBucketReferer (read-only) and return the hotlink
    protection (referer whitelist) configuration.

    Measured contract (oss2 2.19.1):
      * OSS always returns a referer configuration: a bucket without an
        explicit rule reports allow_empty_referer=True and referers=[]
        (the OSS default). The entry script maps referers=[] AND
        black_referers=[] to the semantic state "hotlink protection not
        configured".
      * The oss2 result model ALSO carries black_referers (Referer blacklist)
        and allow_truncate_query_string; both are preserved here (DAL-3):
        dropping them silently corrupts the verdict for blacklist-configured
        buckets, because the official OSS check order is
        empty-referer -> blacklist -> whitelist (blacklist wins).
      * any failure (NoSuchBucket / 403 / network / 5xx) -> raises
    ...
    try:
        ...
        raise _normalize_oss_error(
            exc, f"GetBucketReferer bucket={bucket_name} endpoint={endpoint}")
    return {
        "allow_empty_referer": bool(
            getattr(result, "allow_empty_referer", True)),
        "referers": list(getattr(result, "referers", []) or []),
        "black_referers": list(
            getattr(result, "black_referers", []) or []),
        "allow_truncate_query_string": bool(
            getattr(result, "allow_truncate_query_string", False)),
    }


def get_object_acl(bucket_name: str, object_key: str, endpoint: str,
                   timeout: int = _DEFAULT_TIMEOUT) -> dict:
    """Call OSS GetObjectAcl (read-only) and return one object's ACL.

    Serves the EC 0003-00000005 cause-1 branch (DAL-1): an anonymous probe
    returning 403 on a NON-private bucket must first be attributed to the
    object carrying its own private ACL before any platform-level cause is
    considered. RAM action: oss:GetObjectAcl (see
    references/ram-policies.md).

    Measured contract (oss2 2.19.x): bucket.get_object_acl(key) returns a
    result whose .acl.grant is one of "default", "private",
    "public-read", "public-read-write" ("default" = inherit the bucket
    ACL). Raises OssClientError on any failure (permission, not-found,
    network); the caller degrades with [WARN] and falls back to the
    inference rules of references/ec-0003-00000005-bisection.md.
    """
    bucket = _build_bucket(bucket_name, endpoint, timeout)
    try:
        result = bucket.get_object_acl(object_key)
    except OssClientError:
        raise
    except Exception as exc:  # oss2/requests/socket errors -> normalized
        raise _normalize_oss_error(
            exc,
            f"GetObjectAcl bucket={bucket_name} object={object_key} "
            f"endpoint={endpoint}")
    acl = getattr(result, "acl", None)
    return {"acl": (getattr(acl, "grant", "") or "") if acl is not None
            else ""}


def get_bucket_website(bucket_name: str, endpoint: str,
                       timeout: int = _DEFAULT_TIMEOUT) -> dict:
    """Call OSS GetBucketWebsite (read-only) and return the static website
    hosting configuration.

    Measured contract (oss2 2.19.1):
      * bucket with website hosting -> {"configured": True, "index_file":
        ..., "error_file": ...}
      * bucket WITHOUT website hosting -> oss2 raises a 404 OssError with
        Code=NoSuchWebsiteConfiguration. This is a NORMAL configuration
        state, not an operational error: it is mapped to
        {"configured": False, "code": "NoSuchWebsiteConfiguration"} so the
        entry script reports "static website hosting not configured"
        instead of surfacing an exception.
      * any other failure -> raises OssClientError.
    """
    bucket = _build_bucket(bucket_name, endpoint, timeout)
    try:
        ...
        code = getattr(exc, "code", "") or ""
        status = getattr(exc, "status", 0) or 0
        if code == "NoSuchWebsiteConfiguration" or status == 404:
            return {
                "configured": False,
                "index_file": "",
                "error_file": "",
                "code": "NoSuchWebsiteConfiguration",
                "note": "the bucket has no static website hosting "
                        "configuration; this is a configuration state, "
                        "not an API error",
            }
        raise _normalize_oss_error(
...
def list_bucket_cname(bucket_name: str, endpoint: str,
                      timeout: int = _DEFAULT_TIMEOUT) -> dict:
    """Call OSS ListBucketCname (read-only) and return bound custom domains.

    Measured contract (oss2 2.19.1):
      * bucket with no custom domain bound -> cnames=[] (a finding, not an
        error): the entry script reports "no custom domain bound".
      * any failure -> raises OssClientError with the normalized category.
    """
    bucket = _build_bucket(bucket_name, endpoint, timeout)
    try:
