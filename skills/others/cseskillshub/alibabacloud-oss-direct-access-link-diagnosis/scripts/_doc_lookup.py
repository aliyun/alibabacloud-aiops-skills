#!/usr/bin/env python3
"""
_doc_lookup.py -- runtime official-doc verification for alibabacloud-oss-direct-access-link-diagnosis
=======================================================================
...
# hint is a short label for observability. DAL-8: English keys and the
# video / app-package / web-asset scenario keys are part of the direct-
# access core surface (E-group wording + ticket first-question bank).
SKILL_TOPICS = [
    {"keywords": ["直接访问", "直连", "链接", "direct access", "oss link"],
     "hint": "direct access"},
    {"keywords": ["预览", "下载", "强制下载", "preview", "download"],
     "hint": "preview/download"},
    {"keywords": ["自定义域名", "绑定域名", "域名绑定", "custom domain"],
     "hint": "custom domain"},
    {"keywords": ["备案", "icp"], "hint": "ICP filing"},
    {"keywords": ["防盗链", "referer", "hotlink"], "hint": "hotlink protection"},
    {"keywords": ["视频", "播放", "video"], "hint": "video playback"},
    {"keywords": ["apk", "ipa", "安装包"], "hint": "app package"},
    {"keywords": ["css", "html", "网页", "web page"], "hint": "web assets"},
]
...
_CONSULT_SIGNALS = (
    ...
    "多少", "费用", "收费", "价格", "？", "?",
    # Domain-binding request forms (CNAME / ICP filing trouble reports are
    # usage consultations even without generic signal words).
    "绑定", "域名", "备案", "预览", "下载",
    # DAL-8: core direct-access scenario reports are usage consultations
    # in disguise ("cannot preview / play / access" == "how do I make it
    # work"): video playback, app packages, web assets (css/html), and
    # the English preview/download wording. Without these, E-group
    # sentences like "preview turns into download over the default
    # domain" or "系统生成的视频URL没办法播放" skip the doc lookup even
    # though their topic keywords are present.
    "视频", "播放", "apk", "ipa", "安装包", "css", "html", "网页",
...
assert is_consult_question(
    "preview turns into download over the default domain") is True  # DAL-8: en
assert is_consult_question(
    "系统生成的视频URL没办法播放") is True  # DAL-8: video scenario
assert is_consult_question("css 无法访问") is True  # DAL-8: web-asset scenario
...
def _selftest() -> None:
    ...
    # DAL-8: English wording and scenario wording must activate the lookup
    assert "preview" in _activated_keywords(
        "preview turns into download over the default domain", SKILL_TOPICS)
    assert "视频" in _activated_keywords(
        "系统生成的视频URL没办法播放", SKILL_TOPICS)
    assert "css" in _activated_keywords("css 无法访问", SKILL_TOPICS)
    assert "apk" in _activated_keywords(
        "外网域名下载APK返回 ApkDownloadForbidden", SKILL_TOPICS)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Official-doc lookup self-test for alibabacloud-oss-direct-access-link-diagnosis")
