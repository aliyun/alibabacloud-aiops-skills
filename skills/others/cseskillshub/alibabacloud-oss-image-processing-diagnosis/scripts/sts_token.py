...
def main() -> None:
    ...
    summary = (
        ...
        f"(identity type: {identity_type}). OSS image-processing "
    ...
    suggestions = [
        ...
        "before running the image-processing diagnosis script.",
        "Proceed to image_processing_diagnosis.py with --process and/or "
        "--bucket / --object.",
    ]
    ...
    if args.json:
        ...
        print("Next step       : if this is the account you expect, proceed "
              "with image_processing_diagnosis.py. If not, switch to the "
              "correct credential profile first.")
    print("\nSTATUS: OK")
    print(f"NEXT_ACTION: run image_processing_diagnosis.py with --process "
          f"and/or --bucket / --object to diagnose the image processing "
          f"issue")
