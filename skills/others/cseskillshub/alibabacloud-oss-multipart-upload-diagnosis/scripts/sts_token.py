...
def main() -> None:
    ...
    summary = (
        ...
        f"(identity type: {identity_type}). OSS multipart upload "
    ...
    suggestions = [
        ...
        "before running the multipart upload diagnosis script.",
        "Proceed to multipart_upload_diagnosis.py with --bucket (and "
        "optionally --file-size / --part-size / --endpoint / --region).",
    ]
    ...
    if args.json:
        ...
        print("Next step       : if this is the account you expect, proceed "
              "with multipart_upload_diagnosis.py. If not, switch to the "
              "correct credential profile first.")
    print("\nSTATUS: OK")
    print(f"NEXT_ACTION: run multipart_upload_diagnosis.py with --bucket "
          f"(and optionally --file-size / --part-size / --endpoint / "
          f"--region) to diagnose the multipart upload problem")
