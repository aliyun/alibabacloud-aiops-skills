#!/usr/bin/env python3
"""
_oss_client.py -- Shared OSS control-plane client for Lifecycle diagnosis
==========================================================================
...
values. Only the metadata queries GetBucketInfo, GetBucketVersioning,
GetBucketLifecycle and ListBuckets are ever issued against OSS; no mutating
call exists anywhere in this skill (PutBucketLifecycle and RestoreObject are
deliberately NOT called -- rule creation and restores are described as
...
Measured API behavior (oss2 2.19.1, skillsclienttest role, 2026-08-27):
  * bucket.get_bucket_lifecycle() exists; when the bucket has NO lifecycle
    rule it raises oss2.exceptions.NoSuchLifecycle (HTTP 404,
    code="NoSuchLifecycle") -- this is a valid "no rules configured"
    semantic, NOT an error, and is normalized to an empty rule list below.
  * bucket.get_bucket_versioning() returns an object whose `status` is
    None when versioning was never configured, "Enabled" or "Suspended".
...
_SKILL_NAME = "alibabacloud-oss-lifecycle-runtime-diagnosis"
...
def _build_bucket(bucket_name: str, endpoint: str,
    ...
    Non-conforming bucket names (uppercase, leading/trailing dash, bad
    characters, wrong length) are rejected locally BEFORE any network call
    so the error is attributed as `invalid` instead of surfacing as a
    misleading transport/DNS failure.
    """
    name = (bucket_name or "").strip()
    if not _VALID_BUCKET_NAME_RE.match(name):
        raise OssClientError(
            f"'{name}' is not a valid OSS bucket name; bucket names are "
            "3-63 characters of lowercase letters, digits and hyphens, and "
            "must start/end with a letter or digit",
            category="invalid", code="InvalidBucketName", status=400,
    ...
    return _RetryingHandle(
        oss2.Bucket(auth, endpoint, name, session=session,
                    connect_timeout=timeout),
        bucket_name, endpoint)


_VALID_BUCKET_NAME_RE = re.compile(r"^[a-z0-9][a-z0-9-]{1,61}[a-z0-9]$")
...
def _normalize_oss_error(exc: Exception, operation: str) -> OssClientError:
    ...
    if isinstance(exc, oss2.exceptions.AccessDenied):
        category = "permission"
        hint = ("Check the RAM permission of this caller for the queried "
                "bucket configuration (oss:GetBucketInfo / "
                "oss:GetBucketLifecycle / oss:GetBucketVersioning), or "
    ...
    if isinstance(exc, oss2.exceptions.OssError):
        ...
        if status >= 500:
            ...
            category = "invalid"
        elif code == "NoSuchLifecycle":
            # Should have been consumed by get_bucket_lifecycle(); kept as a
            # safety net so it is never reported as a generic error.
            category = "not_found"
        else:
            category = "unknown"
        return OssClientError(
...
def get_bucket_versioning(bucket_name: str, endpoint: str,
    ...
    """Call OSS GetBucketVersioning (read-only); return the status string.

    Measured: `status` is None when versioning was never configured (mapped
    to ""), "Enabled" or "Suspended". Raises OssClientError on failure.
    """
    bucket = _build_bucket(bucket_name, endpoint, timeout)
    try:
    ...
    return getattr(result, "status", "") or ""


def get_bucket_lifecycle(bucket_name: str, endpoint: str,
                         timeout: int = _DEFAULT_TIMEOUT) -> dict:
    """Call OSS GetBucketLifecycle (read-only) and return normalized rules.

    Measured behavior: when the bucket has no lifecycle rule, oss2 raises
    NoSuchLifecycle (HTTP 404); this is the "no rules configured" semantic
    and is returned as {"rules": [], "configured": False} -- NOT an error.

    Returns:
      {"configured": bool,
       "rules": [ {id, prefix, status, has_expiration, expiration_days,
                   expiration_date, transitions: [{days, storage_class}],
                   abort_multipart_days, has_noncurrent_expiration,
                   noncurrent_days, has_tagging} ... ]}
    Raises OssClientError on any other failure.
    """
    bucket = _build_bucket(bucket_name, endpoint, timeout)
    try:
        ...
        raise _normalize_oss_error(
            exc, f"GetBucketLifecycle bucket={bucket_name}")
    rules = []
    for rule in getattr(lifecycle, "rules", []) or []:
        expiration = getattr(rule, "expiration", None)
        transitions = []
        for tr in getattr(rule, "storage_transitions", None) or []:
            transitions.append({
                "days": getattr(tr, "days", None),
                "storage_class": getattr(tr, "storage_class", "") or "",
            })
        abort = getattr(rule, "abort_multipart_upload", None)
        noncur_exp = getattr(rule, "noncurrent_version_expiration", None)
        tagging = getattr(rule, "tagging", None)
        rules.append({
            ...
            "expiration_date": getattr(expiration, "created_before_date", "")
            if expiration is not None else "",
            "transitions": transitions,
            "abort_multipart_days": getattr(abort, "days", None)
            if abort is not None else None,
            "has_noncurrent_expiration": noncur_exp is not None,
            "noncurrent_days": getattr(noncur_exp, "days", None)
            if noncur_exp is not None else None,
            "has_tagging": bool(tagging and getattr(tagging, "tag_set", None)),
        })
