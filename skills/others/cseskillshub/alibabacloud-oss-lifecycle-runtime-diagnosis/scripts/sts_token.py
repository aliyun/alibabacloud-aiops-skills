...
def main() -> None:
    ...
    summary = (
        ...
        f"(identity type: {identity_type}). OSS lifecycle and archive "
        f"restore diagnostics run by this skill will cover this account only."
    )
    key_findings = [
        f"Account UID: {uid}",
    ...
    suggestions = [
        ...
        "before running the lifecycle diagnosis script.",
        "Proceed to oss_lifecycle_runtime_diagnosis.py with --bucket "
        "(and optionally --scope / --object / --region).",
    ]
    ...
    if args.json:
        ...
        print("Next step       : if this is the account you expect, proceed "
              "with oss_lifecycle_runtime_diagnosis.py. If not, switch to "
              "the correct credential profile first.")
    print("\nSTATUS: OK")
    print(f"NEXT_ACTION: run oss_lifecycle_runtime_diagnosis.py with "
          f"--bucket (and optionally --scope / --object / --region) to "
          f"diagnose the lifecycle rules or archive restore state")
