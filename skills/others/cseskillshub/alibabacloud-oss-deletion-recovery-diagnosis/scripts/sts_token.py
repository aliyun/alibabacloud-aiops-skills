...
def get_caller_identity() -> dict:
    ...
    try:
        ...
        msg = str(e)
        print(f"[ERROR] get-caller-identity failed: {e}", file=sys.stderr)
        if "Plugin" in msg and "not installed" in msg:
            print(
                "[ERROR] Root cause: the required CLI product plugin is not "
                "installed in this environment (e.g. aliyun-cli-sts). This "
                "is an environment capability gap, not a credential problem; "
                "UID cannot be derived via STS here. Ask the platform/runner "
                "owner to install the plugin.",
                file=sys.stderr,
            )
        else:
            print(
                "[ERROR] Next step: check that the aliyun CLI credential "
                "chain is configured (run `aliyun configure`); never pass "
                "AK/SK manually.",
                file=sys.stderr,
            )
        sys.exit(1)
...
def main() -> None:
    ...
    summary = (
        ...
        f"(identity type: {identity_type}). OSS deletion-recovery "
        f"diagnostics run by this skill will cover this account only."
    )
    key_findings = [
        f"Account UID: {uid}",
    ...
    suggestions = [
        ...
        "before running the deletion-recovery diagnosis script.",
        "Proceed to oss_deletion_recovery_diagnosis.py with --bucket (and "
        "optionally --object / --scope / --region).",
    ]
    ...
    if args.json:
        ...
        print("Next step       : if this is the account you expect, proceed "
              "with oss_deletion_recovery_diagnosis.py. If not, switch to "
              "the correct credential profile first.")
    print("\nSTATUS: OK")
    print(f"NEXT_ACTION: run oss_deletion_recovery_diagnosis.py with "
          f"--bucket (and optionally --object / --scope / --region) to "
          f"diagnose the deletion issue")
