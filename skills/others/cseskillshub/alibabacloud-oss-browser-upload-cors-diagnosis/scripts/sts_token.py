...
def main() -> None:
    ...
    summary = (
        ...
        f"(identity type: {identity_type}). Browser-upload / CORS "
    ...
    suggestions = [
        ...
        "before running the browser-upload/CORS diagnosis script.",
        "Proceed to oss_cors_diagnosis.py with --bucket (and optionally "
        "--origin / --region).",
    ]
    ...
    if args.json:
        ...
        print("Next step       : if this is the account you expect, proceed "
              "with oss_cors_diagnosis.py. If not, switch to the correct "
              "credential profile first.")
    print("\nSTATUS: OK")
    print(f"NEXT_ACTION: run oss_cors_diagnosis.py with --bucket (and "
          f"optionally --origin / --region) to diagnose the browser "
          f"direct-upload / CORS configuration")
