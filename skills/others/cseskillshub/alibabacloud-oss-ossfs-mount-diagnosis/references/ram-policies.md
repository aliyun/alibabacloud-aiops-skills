...
This skill performs strictly read-only ossfs / container-mount diagnosis. It
...
|---|---|---|---|
| 1 | `sts:GetCallerIdentity` | aliyun CLI: `aliyun sts get-caller-identity` (plugin mode, lowercase-hyphenated) | Verify the caller identity and derive the account UID (`scripts/sts_token.py`, unconditional in `ossfs_mount_diagnosis.py`) |
| 2 | `oss:GetBucketInfo` | Python oss2 SDK (OSS control-plane, not the POP gateway) | Fetch the bucket location, endpoints, storage class (archive direct-read verdict), owner (`scripts/ossfs_mount_diagnosis.py`) |
| 3 | `oss:ListBuckets` | Python oss2 SDK (OSS control-plane, not the POP gateway) | Fallback bucket-location lookup when GetBucketInfo is denied or the region is unknown (`scripts/ossfs_mount_diagnosis.py`) |
...
- The scripts contain NO mutating OSS call: no object write, no bucket
  configuration change (including the archive-direct-read switch, whose
  mutating API belongs to the user's own manual action), no ACL or policy
  mutation exists anywhere in this skill. The scripts also NEVER execute
  any mount command (ossfs / fusermount / mount / kubectl). The declared
...
- Any request the scripts issue carries a traceable User-Agent
  (`AlibabaCloud-Agent-Skills/alibabacloud-oss-ossfs-mount-diagnosis/{session-id}`).
...
- `Resource: "*"` is used because GetBucketInfo / ListBuckets must cover any
  ...
  narrowed to the ARN of a specific bucket (no wildcards) when the diagnosis
...
| Data exposed | Bucket location, endpoints, storage class, ACL label, creation date, owner id - metadata only, never object content |
...
- Grant only the three actions above; do not attach `AliyunOSSFullAccess` or
