#!/usr/bin/env python3
"""
_doc_lookup.py -- runtime official-doc verification for alibabacloud-oss-ossfs-mount-diagnosis
=======================================================================
...
# hint is a short label for observability. The Chinese keywords are match
# tokens rather than prose: customers ask in Chinese ("ossfs 挂载失败"), so each
# topic carries its Chinese synonyms next to the English ones. MUST 4.1.2
# governs the language of SKILL.md and references/*.md, not this matching data.
SKILL_TOPICS = [
    {"keywords": ["ossfs", "挂载", "mount"], "hint": "ossfs mount"},
    {"keywords": ["ecs", "云主机", "服务器"], "hint": "ECS mount"},
    {"keywords": ["transport endpoint", "not connected"], "hint": "mount lost"},
    {"keywords": ["备份", "费用"], "hint": "mounted usage"},
    {"keywords": ["授权", "ecsramrole", "权限"], "hint": "mount auth"},
]

# Configuration/usage consultation signal words (Step B condition 2). Chinese
# interrogative/consultation particles are part of the signal set because the
# customer question arrives in Chinese; they are match tokens, not prose (see
# the note on SKILL_TOPICS above re: the scope of MUST 4.1.2).
_CONSULT_SIGNALS = (
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Official-doc lookup self-test for alibabacloud-oss-ossfs-mount-diagnosis")
