...
def main() -> None:
    ...
    summary = (
        ...
        f"(identity type: {identity_type}). OSS ossfs mount diagnostics run "
    ...
    suggestions = [
        ...
        "before running the ossfs mount diagnosis script.",
        "Proceed to ossfs_mount_diagnosis.py with --bucket (and optionally "
        "--endpoint / --symptom / --platform).",
    ]
    ...
    if args.json:
        ...
        print("Next step       : if this is the account you expect, proceed "
              "with ossfs_mount_diagnosis.py. If not, switch to the correct "
              "credential profile first.")
    print("\nSTATUS: OK")
    print(f"NEXT_ACTION: run ossfs_mount_diagnosis.py with --bucket (and "
          f"optionally --endpoint / --symptom / --platform) to diagnose the "
          f"ossfs / container mount problem")
