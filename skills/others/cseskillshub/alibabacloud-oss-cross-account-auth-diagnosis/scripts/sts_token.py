...
def main() -> None:
    ...
    summary = (
        ...
        f"(identity type: {identity_type}). Cross-account authorization "
        f"diagnostics run by this skill compare this UID with the bucket "
        f"owner UID and cover this account's side only."
    )
    key_findings = [
        f"Account UID: {uid}",
    ...
    suggestions = [
        "Confirm this UID matches the account involved in the cross-account "
        "scenario (source / destination / bucket-owner side) before running "
        "the diagnosis script.",
        "Proceed to oss_cross_account_auth_diagnosis.py with --bucket (and "
        "optionally --scenario / --peer-uid / --role-name / --region).",
    ]
    ...
    if args.json:
        ...
        print("Next step       : if this is the account you expect, proceed "
              "with oss_cross_account_auth_diagnosis.py. If not, switch to "
              "the correct credential profile first.")
    print("\nSTATUS: OK")
    print(f"NEXT_ACTION: run oss_cross_account_auth_diagnosis.py with "
          f"--bucket (and optionally --scenario / --peer-uid / --role-name "
          f"/ --region) to diagnose the cross-account authorization issue")
