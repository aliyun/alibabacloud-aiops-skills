#!/usr/bin/env python3
"""
_oss_client.py -- Shared OSS control-plane client for cross-account auth
=========================================================================
...
values. Only the three metadata queries GetBucketInfo, GetBucketPolicy and
ListBuckets are ever issued against OSS; no mutating call (no PutBucketPolicy,
no RAM/trust-policy change, no replication configuration) exists anywhere in
this skill -- authorization fixes are emitted as templates for the user to
apply manually.
...
Channels (measured, finalized):
  * OSS control-plane metadata -> Python oss2 SDK (>= 2.19.0, < 3). The
    ...
    channel. Measured method names on oss2 2.19.1 Bucket:
      ...
      - bucket.get_bucket_policy()        -> .policy: raw JSON string;
                                              raises NoSuchBucketPolicy
                                              (HTTP 404) when the bucket
...
_SKILL_NAME = "alibabacloud-oss-cross-account-auth-diagnosis"
...
class OssClientError(RuntimeError):
    ...
    def __init__(self, message: str, category: str = "unknown",
        ...
        self.status = status
        self.hint = hint
        # OSS new-style error code (e.g. 0003-00000201) and the raw response
        # body, captured so the entry script can extract the AccessDeniedDetail
        # / EncodedDiagnosticMessage for the RAM 403 localization main path
        # (G3-5). body stays an internal attribute (never echoed verbatim into
        # the report); ec is surfaced in to_dict().
        self.ec = ec
        self.body = body
...
def _normalize_oss_error(exc: Exception, operation: str) -> OssClientError:
    ...
    if _rid and _rid not in msg:
        ...
        low = msg.lower()
    # OSS new-style error code (EC 0003-*) and the raw response body: oss2
    # exposes `.ec` and `.body` on every OssError. The body carries the
    # <AccessDeniedDetail> block (PolicyType / NoPermissionType / AuthAction /
    # EncodedDiagnosticMessage) that drives the RAM 403 localization main path
    # (G3-5); the entry script parses it via extract_access_denied_detail().
    _ec = str(getattr(exc, "ec", "") or "").strip()
    _body = str(getattr(exc, "body", "") or "")
    ...
    if isinstance(exc, oss2.exceptions.NoSuchBucket):
        return OssClientError(
            ...
            hint="Verify the bucket name spelling and the owning account; "
                 "bucket names are global and case-sensitive lowercase. A "
                 "cross-account bucket is never visible via GetBucketInfo "
                 "from an account that does not own it.",
        )
    if isinstance(exc, oss2.exceptions.AccessDenied):
        ...
        # ownership problem. In a cross-account scenario the missing grant is
        # the peer account's Bucket Policy, so lead with the pre-check action
        # and keep the cross-account grant as the concrete remediation.
        if "does not belong to you" in low:
            hint = ("Access denied (403) on a client-side pre-check: per the "
                    ...
                    "For cross-account access the owning account must grant "
                    "those bucket-level actions via Bucket Policy; a "
                    "wrong-region endpoint is a secondary hypothesis only.")
...
def get_bucket_info(bucket_name: str, endpoint: str,
    ...
    owner_id is the owning account UID -- the key fact used to decide
    whether the caller's account owns the bucket or this is a
    cross-account access situation.
...
def get_bucket_policy(bucket_name: str, endpoint: str,
                      timeout: int = _DEFAULT_TIMEOUT) -> dict:
    """Call OSS GetBucketPolicy (read-only) and return normalized fields.

    Returns {"state": "present"|"not_configured", "policy": <raw JSON str>}.
    Measured on oss2 2.19.1: a bucket without any Bucket Policy raises
    oss2.exceptions.NoSuchBucketPolicy (HTTP 404, code NoSuchBucketPolicy);
    this is a NORMAL configuration fact ("policy not configured") and is
    NOT recorded as an error -- the caller treats it as the
    not_configured semantic. Any other failure raises OssClientError with
    a normalized category (permission / network / ...).
    """
    bucket = _build_bucket(bucket_name, endpoint, timeout)
    try:
        result = bucket.get_bucket_policy()
    except oss2.exceptions.NoSuchBucketPolicy:
        # 404 NoSuchBucketPolicy == no policy configured (measured).
        return {"state": "not_configured", "policy": ""}
    except OssClientError:
        raise
    except Exception as exc:
        # oss2 raises NoSuchBucket / AccessDenied / network errors here;
        # defensive: NoSuchBucketPolicy subclasses may also arrive wrapped.
        code = getattr(exc, "code", "") or ""
        if code == "NoSuchBucketPolicy":
            return {"state": "not_configured", "policy": ""}
        raise _normalize_oss_error(
            exc, f"GetBucketPolicy bucket={bucket_name} endpoint={endpoint}")
    policy = (getattr(result, "policy", "") or "").strip()
    return {"state": "present" if policy else "not_configured",
            "policy": policy}
...
def resolve_uid() -> str:
    ...
    a traceability label for this read-only skill. Note: for an assumed-role
    (STS) credential, AccountId is the UID of the account OWNING the role,
    which is exactly the account relevant for cross-account comparison.
    """
    try:
...
# ---------------------------------------------------------------------------
# RAM 403 localization main path (G3-5): decode an EncodedDiagnosticMessage
# ---------------------------------------------------------------------------

def decode_diagnostic_message(encoded: str, timeout: int = _CLI_TIMEOUT) -> dict:
    """Decode an AccessDeniedDetail.EncodedDiagnosticMessage via the aliyun CLI
    `ram decode-diagnostic-message` (RAM 2015-05-01, read-only action
    `ram:DecodeDiagnosticMessage`).

    This is the OFFICIAL 403 localization main path (source:
    help.aliyun.com/zh/oss/user-guide/0003-00000201 and
    /zh/ram/support/how-to-troubleshoot-an-access-denied-error): the encoded
    message carried by a denied response is decoded to reveal the exact
    AuthAction / AuthResource / AuthPrincipal / NoPermissionPolicyType /
    AuthConditions / MatchedPolicies that caused the deny.

    NEVER raises and NEVER prints the encoded value. Returns a structured dict
    so the caller degrades gracefully. MEASURED (2026-09-07, account
    1552974654746705): the evaluation role `skillsclienttest` LACKS
    `ram:DecodeDiagnosticMessage` -- RAM answers 403 NoPermission, which is
    exactly the official "you lack the diagnosis permission" branch (hand the
    EncodedDiagnosticMessage to an account administrator who holds the
    permission, or paste it into the RAM permission-diagnosis console page).
    The default RAM user (which holds the permission) reaches parameter
    validation instead, confirming the classification is permission-based.

    Result dict keys:
      available (bool)        -- True only when the decode succeeded (2xx)
      decoded (dict)          -- DecodedDiagnosticMessage when available
      reason (str)            -- error code when unavailable (NoPermission /
                                 EncodedMessageExpire / InvalidParameter / ...)
      needs_permission (str)  -- the RAM action required (for guidance text)
      message (str)           -- short human-readable failure detail
      request_id (str)        -- RAM RequestId when present
    """
    result = {
        "available": False, "decoded": {}, "reason": "",
        "needs_permission": "ram:DecodeDiagnosticMessage",
        "message": "", "request_id": "",
    }
    encoded = (encoded or "").strip()
    if not encoded:
        result["reason"] = "EmptyEncodedMessage"
        result["message"] = ("no EncodedDiagnosticMessage was present in the "
                             "denied response; nothing to decode")
        return result
    if shutil.which("aliyun") is None:
        result["reason"] = "CliMissing"
        result["message"] = ("aliyun CLI not found on PATH; cannot call "
                             "ram:DecodeDiagnosticMessage")
        return result
    cmd = ["aliyun", "ram", "decode-diagnostic-message",
           "--encoded-diagnostic-message", encoded,
           "--user-agent", user_agent()]
    try:
        ...
        result["message"] = ("ram decode-diagnostic-message timed out after "
                             f"{timeout}s")
        return result
    out = _ANSI_ESCAPE_RE.sub("", proc.stdout or "").strip()
    err = _ANSI_ESCAPE_RE.sub("", proc.stderr or "").strip()
    if proc.returncode == 0 and out:
        try:
            data = json.loads(out)
        except json.JSONDecodeError:
            data = {}
        decoded = data.get("DecodedDiagnosticMessage") or {}
        result["request_id"] = str(data.get("RequestId") or "")
        if decoded:
            result["available"] = True
            result["decoded"] = decoded
            return result
        result["reason"] = "EmptyDecode"
        result["message"] = "the CLI returned no DecodedDiagnosticMessage"
        return result
    # Failure path: classify the RAM error code from the CLI SDKError text.
    # The encoded value is NEVER echoed back into the message.
    blob = err or out
    m = re.search(r"Code:\s*([A-Za-z.]+)", blob)
    result["reason"] = m.group(1) if m else "DecodeFailed"
    rid = re.search(r"request id:\s*([0-9A-Fa-f-]+)", blob)
    result["request_id"] = rid.group(1) if rid else ""
    low = blob.lower()
    if "nopermission" in low or "do not have the required permission" in low:
        result["message"] = ("the current credential LACKS "
                             "ram:DecodeDiagnosticMessage (RAM answered 403 "
                             "NoPermission) -- the official "
                             "lack-diagnosis-permission branch")
    else:
        result["message"] = ("ram decode-diagnostic-message failed "
                             f"({result['reason']})")
    return result
