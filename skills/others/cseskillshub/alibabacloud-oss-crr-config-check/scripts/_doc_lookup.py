#!/usr/bin/env python3
"""
_doc_lookup.py -- runtime official-doc verification for alibabacloud-oss-crr-config-check
=======================================================================
...
SKILL_TOPICS = [
    {"keywords": ["跨区域复制", "区域复制", "数据复制", "复制规则", "复制到"], "hint": "cross-region replication"},
    {"keywords": ["replication", "crr"], "hint": "CRR"},
    {"keywords": ["同步", "增量", "迁移", "在线迁移"], "hint": "sync/migration"},
    {"keywords": ["延迟", "历史数据", "关闭"], "hint": "CRR behavior"},
    # C6 symptom vocabulary (kept in sync with crr_config_check.SYMPTOM_ROUTING
    # so the doc-lookup leg activates on the same customer wording the symptom
    # router matches: progress stuck / target empty / slow / not effective).
    {"keywords": ["复制进度", "目标桶", "复制慢", "复制延迟", "复制失败",
                  "复制不同步", "不生效", "传输加速"], "hint": "CRR symptoms"},
]
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Official-doc lookup self-test for alibabacloud-oss-crr-config-check")
