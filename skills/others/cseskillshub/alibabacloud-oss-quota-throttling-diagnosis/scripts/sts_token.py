...
def get_caller_identity() -> dict:
    ...
    try:
        ...
        print(
            ...
            "is configured (run `aliyun configure`); never pass raw "
            "credential values manually.",
            file=sys.stderr,
        )
        sys.exit(1)
...
def main() -> None:
    ...
    summary = (
        ...
        f"(identity type: {identity_type}). OSS quota and throttling "
    ...
    suggestions = [
        ...
        "before running the quota/throttling diagnosis script.",
        "Proceed to oss_quota_diagnosis.py with --bucket (and optionally "
        "--symptom / --error-code / --peak-qps / --peak-bandwidth-gbps).",
    ]
    ...
    if args.json:
        ...
        print("Next step       : if this is the account you expect, proceed "
              "with oss_quota_diagnosis.py. If not, switch to the correct "
              "credential profile first.")
    print("\nSTATUS: OK")
    print(f"NEXT_ACTION: run oss_quota_diagnosis.py with --bucket (and "
          f"optionally --symptom / --error-code / --peak-qps / "
          f"--peak-bandwidth-gbps) to assess quota and throttling")
