#!/usr/bin/env python3
"""
_oss_client.py -- Shared OSS control-plane client for quota/throttling diagnosis
=================================================================================
...
_SKILL_NAME = "alibabacloud-oss-quota-throttling-diagnosis"
...
class OssClientError(RuntimeError):
    ...
    category values (stable contract consumed by the diagnosis entry):
      ...
      credentials        -- invalid/expired key pair or session token, signature mismatch
    ...
    def get_credentials(self) -> Credentials:
        ...
        if not access_key_id or not access_key_secret:
            raise OssClientError(
                ...
                "configure the default credential chain first, never pass "
                "raw credential values manually",
...
def get_caller_identity(timeout: int = _CLI_TIMEOUT) -> dict:
    ...
    if shutil.which("aliyun") is None:
        raise CliError(
            ...
            "(`aliyun configure`) -- never pass raw credential values manually",
            code="CliMissing",
        )
