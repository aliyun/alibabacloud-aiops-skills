...
def main() -> None:
    ...
    summary = (
        ...
        f"(identity type: {identity_type}). OSS transfer acceleration "
    ...
    suggestions = [
        ...
        "before running the transfer acceleration diagnosis script.",
        "Proceed to oss_transfer_acceleration_diagnosis.py with --bucket "
        "(and optionally --endpoint / --client-location / --scope).",
    ]
    ...
    if args.json:
        ...
        print("Next step       : if this is the account you expect, proceed "
              "with oss_transfer_acceleration_diagnosis.py. If not, switch "
              "to the correct credential profile first.")
    print("\nSTATUS: OK")
    print("NEXT_ACTION: run oss_transfer_acceleration_diagnosis.py with "
          "--bucket (and optionally --endpoint / --client-location / "
          "--scope) to diagnose OSS transfer acceleration")
