#!/usr/bin/env python3
"""
_oss_client.py -- Shared OSS control-plane client for Endpoint diagnosis
=========================================================================
...
values. Only the two metadata queries GetBucketInfo and ListBuckets are ever
issued against OSS; no mutating call exists anywhere in this skill.
...
_SKILL_NAME = "alibabacloud-oss-endpoint-internal-diagnosis"
...
def _normalize_oss_error(exc: Exception, operation: str) -> OssClientError:
    ...
    if isinstance(exc, oss2.exceptions.AccessDenied):
        category = "permission"
        hint = ("Check RAM permission oss:GetBucketInfo for this caller, or "
    ...
    if isinstance(exc, oss2.exceptions.OssError):
        ...
        # "no such host / internal endpoint outside the cloud" guidance.
...
def list_bucket_cname(bucket_name: str, endpoint: str,
    ...
    """Call OSS ListCname (read-only) and return the bucket's bound custom domains.

    RAM action: oss:ListCname (verified via api.aliyun.com meta Oss/2019-05-17
    ListCname, read-only). Returns a list of dicts:
      {"domain": <custom domain>, "status": <Enabled|Disabled>,
       "last_modified": <ts>, "has_certificate": <bool>,
       "cert_status": <Enabled|Disabled|"">}
    Official Status enum is Enabled|Disabled only (ListCname response element
    table); the OSS console's "待检测" (pending verification) is a console-side
    CnameToken ownership-verification state, NOT an API Status value -- a domain
    still pending verification is typically absent from ListCname until verified.
    `has_certificate`/`cert_status` come from the optional Certificate node and
    ground the HTTPS branch (endpoint-rules.md §3.3 step 3). A custom domain
    bound on the bucket appears here; an empty list means the domain the user
    reported is NOT bound to this bucket (§3.3 step 2). Raises OssClientError
    with a normalized category on failure (permission / not_found / network ...),
    ...
    try:
        ...
        raise _normalize_oss_error(
            exc, f"ListCname bucket={bucket_name} endpoint={endpoint}")
    out = []
    for c in (getattr(result, "cname", []) or []):
        cert = getattr(c, "certificate", None)
        out.append({
            "domain": (getattr(c, "domain", "") or "").strip().lower(),
            "status": getattr(c, "status", "") or "",
            "last_modified": getattr(c, "last_modified", "") or "",
            "has_certificate": cert is not None,
            "cert_status": (getattr(cert, "status", "") or "") if cert else "",
        })
    return out


_DNS_TIMEOUT = 5   # seconds, hard bound on the best-effort resolution probe


def resolve_cname_target(domain: str, timeout: int = _DNS_TIMEOUT) -> dict:
    """Best-effort DNS resolution of a custom domain (NOT a cloud API call).

    Grounds the CNAME troubleshooting path (endpoint-rules.md §3.3 step 1):
    a custom domain that fails to resolve reproduces the customer-side
    `UnknownHostException` / "no such host" symptom, and a domain that
    resolves reveals whether it points at the bucket's OSS endpoint.

    This uses the OS resolver only -- it issues NO request to any Alibaba
    Cloud API and needs NO credential / RAM permission. It is fully
    degradable: any failure (NXDOMAIN, timeout, resolver error) returns
    resolved=False with the error text instead of raising. The lookup runs
    on a daemon thread bounded by `timeout` so a black-holed resolver can
    never hang the diagnosis.

    Returns {"resolved": bool, "canonical": str, "aliases": [..],
             "addresses": [..], "error": str}.
    """
    dom = (domain or "").strip().lower().rstrip(".")
    out = {"resolved": False, "canonical": "", "aliases": [],
           "addresses": [], "error": ""}
    if not dom:
    ...
    def _work():
        try:
            box["result"] = socket.gethostbyname_ex(dom)
        except Exception as exc:  # gaierror (NXDOMAIN) / herror / timeout
            box["error"] = exc
    ...
    t.join(timeout)
    if t.is_alive():
        out["error"] = f"DNS resolution timed out after {timeout}s"
        return out
    if "error" in box:
        out["error"] = f"DNS resolution failed: {box['error']}"
        return out
    canonical, aliases, ips = box.get("result", ("", [], []))
    out["resolved"] = bool(ips)
    out["canonical"] = canonical or ""
    out["aliases"] = [a for a in (aliases or [])][:10]
    out["addresses"] = [a for a in (ips or [])][:10]
    return out


# --- inline boundary assertions for resolve_cname_target (no network) ---
# invalid: empty domain short-circuits without touching the resolver
_r_empty = resolve_cname_target("")
assert _r_empty["resolved"] is False and _r_empty["error"] == "empty domain", \
    "resolve_cname_target('') must degrade to resolved=False"
assert resolve_cname_target(None)["error"] == "empty domain"  # invalid: None
# boundary: the returned shape is always complete regardless of resolution
assert set(_r_empty.keys()) == {"resolved", "canonical", "aliases",
                                "addresses", "error"}  # stable contract shape
# --- end resolve_cname_target inline assertions ---
