...
def main() -> None:
    ...
    summary = (
        ...
        f"(identity type: {identity_type}). OSS endpoint diagnostics run by "
        f"this skill will cover this account only."
    )
    key_findings = [
        f"Account UID: {uid}",
    ...
    suggestions = [
        ...
        "before running the endpoint diagnosis script.",
        "Proceed to oss_endpoint_diagnosis.py with --bucket (and optionally "
        "--endpoint / --region).",
    ]
    ...
    if args.json:
        ...
        print("Next step       : if this is the account you expect, proceed "
              "with oss_endpoint_diagnosis.py. If not, switch to the correct "
              "credential profile first.")
    print("\nSTATUS: OK")
    print(f"NEXT_ACTION: run oss_endpoint_diagnosis.py with --bucket (and "
          f"optionally --endpoint / --region) to diagnose the OSS endpoint")
