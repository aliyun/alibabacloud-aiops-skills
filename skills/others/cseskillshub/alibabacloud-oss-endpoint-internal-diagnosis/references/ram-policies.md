...
This skill performs strictly read-only endpoint diagnosis. It requires
exactly four RAM actions. No wildcards (`*`) are used in any action name.

## Required actions

| # | RAM Action | Invocation channel | Purpose |
|---|---|---|---|
| 1 | `sts:GetCallerIdentity` | aliyun CLI: `aliyun sts get-caller-identity` (plugin mode, lowercase-hyphenated) | Verify the caller identity and derive the account UID (`scripts/sts_token.py`, unconditional in `oss_endpoint_diagnosis.py`) |
| 2 | `oss:GetBucketInfo` | Python oss2 SDK (OSS control-plane, not the POP gateway) | Fetch the bucket location, public/internal endpoints, storage class, ACL (`scripts/oss_endpoint_diagnosis.py`) |
| 3 | `oss:ListBuckets` | Python oss2 SDK (OSS control-plane, not the POP gateway) | Fallback bucket-location lookup when GetBucketInfo is denied or the region is unknown (`scripts/oss_endpoint_diagnosis.py`) |
| 4 | `oss:ListCname` | Python oss2 SDK (OSS control-plane, not the POP gateway) | Custom-domain (CNAME) binding check; issued only when `--endpoint` classifies as a custom domain, fully degradable (`scripts/_oss_client.py list_bucket_cname`). Official meta Oss/2019-05-17 `ListCname`, read-only |

All four are read-only Get/List-class actions. They never modify any
resource, object, policy, or configuration. The CNAME path additionally runs
an OS-level DNS resolution probe (`resolve_cname_target`), which is not a
cloud API and needs no RAM action.
...
- `oss:GetBucketInfo`, `oss:ListBuckets`, and `oss:ListCname` return metadata
  only; object content is never read or written by this skill. `ListCname`
  returns the bound custom domains and their Status (Enabled/Disabled) plus
  certificate presence - never the certificate private key.
- Any request the scripts issue carries a traceable User-Agent
  (`AlibabaCloud-Agent-Skills/alibabacloud-oss-endpoint-internal-diagnosis/{session-id}`).
...
- `Resource: "*"` is used because GetBucketInfo / ListBuckets / ListCname must
  ...
  be narrowed to the ARN of a specific bucket (no wildcards) when the diagnosis
  scope is known in advance.
