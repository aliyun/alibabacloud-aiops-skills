#!/usr/bin/env python3
"""
_doc_lookup.py -- runtime official-doc verification for alibabacloud-oss-access-log-trace-diagnosis
=======================================================================
...
#
# The first four topics are the access-log enablement / trace domain. The rest
# cover the root-cause diagnosis domain added on top of it: EC error codes,
# authorization denials, bucket-policy grammar, signature failures and the
# object-existence family. Both Chinese and English wording are listed because
# customers raise these in either language.
SKILL_TOPICS = [
    # -- access-log enablement and tracing --
    {"keywords": ["访问日志", "日志", "访问记录", "实时日志", "访问明细",
                  "access log", "access logs", "realtime log", "real-time log",
                  "logstore", "oss-log-store"], "hint": "access log"},
    {"keywords": ["下载日志", "请求记录", "请求明细", "日志转存", "日志投递",
                  "log delivery", "log shipping"], "hint": "log export"},
    {"keywords": ["谁访问", "哪些地址", "ip", "谁下载", "谁删除", "溯源",
                  "who accessed", "which ip", "trace", "request id",
                  "requestid"], "hint": "access trace"},
    {"keywords": ["流量", "下行流量", "traffic", "outbound"],
     "hint": "traffic attribution"},
    # -- EC error codes and error responses --
    {"keywords": ["ec码", "ec 码", "错误码", "error code", "错误响应",
                  "error response", "recommenddoc"], "hint": "error code"},
    # -- authorization denials --
    {"keywords": ["403", "accessdenied", "access denied", "权限不足",
                  "拒绝访问", "无权", "permission", "denied"],
     "hint": "access denied"},
    {"keywords": ["bucket policy", "授权策略", "策略", "policy", "deny",
                  "ram policy", "显式拒绝"], "hint": "policy"},
    {"keywords": ["acl", "公共读", "私有", "阻止公共访问", "public access",
                  "public-read", "anonymous", "匿名"], "hint": "acl and public access"},
    {"keywords": ["防盗链", "referer", "hotlink", "白名单"],
     "hint": "hotlink protection"},
    {"keywords": ["sts", "临时凭证", "assumerole", "角色", "role",
                  "security token", "信任策略", "trust policy"],
     "hint": "sts and roles"},
    # -- signature family --
    {"keywords": ["signaturedoesnotmatch", "签名", "signature", "预签名",
                  "presigned", "stringtosign", "canonicalrequest",
                  "requesttimetooskewed", "时间偏差", "clock skew"],
     "hint": "signature"},
    # -- existence family --
    {"keywords": ["404", "nosuchkey", "nosuchbucket", "文件不存在",
                  "invalidbucketname", "bucket 名称", "bucket name",
                  "not exist"], "hint": "existence"},
    # -- object behaviour --
    {"keywords": ["强制下载", "预览", "content-disposition", "图片处理",
                  "image processing", "x-oss-process", "魔数", "magic bytes"],
     "hint": "object behaviour"},
    {"keywords": ["删除", "delete", "生命周期", "lifecycle", "版本控制",
                  "versioning", "delete marker"], "hint": "deletion and lifecycle"},
]
...
_CONSULT_SIGNALS = (
    ...
    "多少", "费用", "收费", "价格", "？", "?",
    # Access-log-domain request forms (log export / attribution requests
    # are usage consultations even without generic signal words).
    "日志", "导出", "检查", "排查", "溯源", "追踪", "流量",
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Official-doc lookup self-test for alibabacloud-oss-access-log-trace-diagnosis")
