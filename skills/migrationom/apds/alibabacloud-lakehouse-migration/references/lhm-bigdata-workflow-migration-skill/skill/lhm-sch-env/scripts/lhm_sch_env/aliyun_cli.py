...
def _kebab_case(name: str) -> str:
    """camelCase/snake_case -> kebab-case（dataSourceName -> data-source-name, page_index -> page-index）。"""
    # 先处理 snake_case（下划线转连字符）
    result = name.replace("_", "-")
    # 再处理 camelCase（大写字母前加连字符）
...
def _build_user_agent(skill_name: str = "lhm-sch-env") -> str:
