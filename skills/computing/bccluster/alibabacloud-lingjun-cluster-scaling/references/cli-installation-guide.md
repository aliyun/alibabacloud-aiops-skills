...
> **⚠️ Transient Failure Retry (MANDATORY)** ｜ This file belongs to the **CLI installation / diagnosis phase**: at this point `safe_aliyun` is not yet loaded, and retries would mask the core diagnostic signal "can the CLI itself connect", so **this file's examples exceptionally keep bare `aliyun ...`**. After the CLI is installed, all business calls must go through the `safe_aliyun` wrapper of [SKILL.md → Transient Failure Retry (MANDATORY)](../SKILL.md) (implementation skeleton in [edge-cases.md → Appendix B.4](./edge-cases.md)).
...
## Plugins for Lingjun Cluster Scaling Skill

This SKILL orchestrates three Alibaba Cloud OpenAPI namespaces (aligned with design doc §1.2). You must install **all three** plugins below before running any skill command; otherwise `unknown product` errors will be returned.

| Plugin | CLI Namespace | OpenAPI Product | Used For |
|---|---|---|---|
| `eflo-controller` | `aliyun eflo-controller ...` | Eflo-Controller | Clusters / node groups / nodes / machine types / images (`list-clusters`, `extend-cluster`, `shrink-cluster`, `create-node-group`, `update-node-group`, `list-node-groups`, `describe-node`, `delete-node`, `change-node-group`, `list-machine-types`, `list-images`, etc.) |
| `bssopenapi` | `aliyun bssopenapi ...` | BssOpenApi (Lingxiao) | Subscription-billed node purchase (`CreateInstance`) and order status polling (`QueryOrders`) during node provisioning |
| `ecs` | `aliyun ecs ...` | ECS | Query ECS custom images (`DescribeImages`) when creating nodes with a user-provided image |
...
**Verify plugins are ready**:

```bash
# List installed plugins — should contain eflo-controller, bssopenapi, ecs
aliyun plugin list

# Smoke test each namespace (requires RAM permissions, see ram-policies.md)
aliyun eflo-controller list-clusters --region cn-hangzhou
aliyun bssopenapi query-account-balance
aliyun ecs describe-regions
```

**Upgrade existing plugins** (recommended periodically to pick up new APIs):

```bash
aliyun plugin upgrade --name eflo-controller
aliyun plugin upgrade --name bssopenapi
aliyun plugin upgrade --name ecs
```

> **Minimum CLI version**: Aliyun CLI **3.3.3+** is required. Older versions do not expose the plugin subsystem and cannot load `eflo-controller` / `bssopenapi`. Check with `aliyun version` before installing plugins.

> **RAM permissions**: The RAM principal used by the CLI must cover `eflo:*`, `bss:*` and `ecs:DescribeImages`. See [RAM Policies](./ram-policies.md) for the least-privilege permission sets tailored to this skill.
...
1. **Install plugins** for services you need (v3.3.3+ supports all published product plugins):
   ```bash
   # Plugins required by the Lingjun Cluster Scaling skill
   aliyun plugin install --name eflo-controller
   aliyun plugin install --name bssopenapi
   aliyun plugin install --name ecs

   # Other commonly used plugins
   aliyun plugin install --names vpc rds
...
2. **Explore commands**:
   ```bash
   aliyun eflo-controller --help
   aliyun bssopenapi --help
   aliyun ecs --help
   ```
...
3. **Read documentation**:
   ...
   - [RAM Policies](./ram-policies.md)

## References

- Official Documentation: https://help.aliyun.com/zh/cli/
- Eflo-Controller OpenAPI: https://help.aliyun.com/zh/pai/developer-reference/api-eflo-controller-2022-12-15-overview
- BssOpenApi (Lingxiao) OpenAPI: https://help.aliyun.com/zh/ssi/developer-reference/api-bssopenapi-2017-12-14-overview
- ECS OpenAPI: https://help.aliyun.com/zh/ecs/developer-reference/api-ecs-2014-05-26-overview
