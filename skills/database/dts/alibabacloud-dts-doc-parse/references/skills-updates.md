...
- Installations that contain a valid `references/manifest.json` participate by default. Only DTS Skills already installed in the selected directory are checked; name, version, and minimum CLI come from that file. A package leaf that is a symlink is resolved and checked against the real directory; the link target is never replaced. Catalog checks use this CLI's Skills source. Directories without a valid `references/manifest.json` stay `unmanaged`. Reinstall an old package once from the joint release to identify it.
- The CLI check uses this CLI distribution's `releases/latest/manifest.json`. A newer CLI is installed first, then the new `dtscli` continues. Skills are read from each package's own `releases/<skill-name>/manifest.json`. Caches are separate for each Skill source; Agents share the cache for the same Skill. Test and production caches are isolated by source.
