#!/usr/bin/env python3
"""SQL lifecycle implementation for Drafts, validation, and deployment."""
...
# ═══════════════════════════════════════════════════════════════════════════
# Folder management (four commands)
# ═══════════════════════════════════════════════════════════════════════════
...
def cmd_delete_folder(args):
    ...
    chk = require_confirmation(
        "delete_folder",
        f"Delete Folder '{args.folder_id}'. Only an empty Folder can be deleted.",
        args.confirm,
    )
    if chk:
        return output(chk, args.output)
...
def cmd_create_draft(args):
    ...
    if args.body_json:
        ...
        # The API requires engine_version.
...
def cmd_delete_draft(args):
    ...
    chk = require_confirmation(
        "delete_draft",
        f"Delete Draft '{args.draft_id}'. A Draft with an active Deployment cannot be deleted.",
        args.confirm,
    )
    if chk:
        return output(chk, args.output)
...
# ═══════════════════════════════════════════════════════════════════════════
# Validation and deployment (five commands)
# ═══════════════════════════════════════════════════════════════════════════
...
def register(subparsers):
    """Register SQL development and Draft subcommands."""
    ...
    def _draft_create(p):
        ...
        p.add_argument(
            "--sql",
            dest="content",
            help="Alias for --content that passes SQL content (backward compatibility)",
        )
        ...
        p.add_argument("--engine_version", help="Engine version (for example, vvr-8.0.11-flink-1.17)")
        p.add_argument("--body_json", help="Complete Draft request body JSON (advanced)")
        p.add_argument("--confirm", action="store_true", help="Skip interactive confirmation")
    ...
    _add("get_draft_lock", "Get the Draft edit-lock status", cmd_get_draft_lock, _draft_id)

    # ── Validation and deployment ──────────────────────────────────────

    def _validate_draft(p):
        p.add_argument("--draft_id", required=True, help="Draft ID to validate")
        p.add_argument("--deployment_target", help="Deployment Target name used for validation")
    ...
    def _validate_sql(p):
        ...
        p.add_argument(
            "--sql",
            dest="statement",
            help="Alias for --statement (backward compatibility)",
        )
    ...
    def _deploy_draft(p):
        ...
        p.add_argument(
            "--deployment_target_mode",
            default="PER_JOB",
            help="Deployment Target mode (default: PER_JOB)",
        )
        p.add_argument("--confirm", action="store_true", help="Skip interactive confirmation")

    _add("validate_draft", "Validate a Draft thoroughly (asynchronous)", cmd_validate_draft, _validate_draft)
    _add("get_validate_result", "Get a validation result by ticket ID", cmd_get_validate_result, _ticket)
    _add("validate_sql", "Validate SQL syntax quickly", cmd_validate_sql, _validate_sql)
    _add("deploy_draft", "Deploy a Draft to production (asynchronous)", cmd_deploy_draft, _deploy_draft)
    _add("get_deploy_result", "Get a deployment result by ticket ID", cmd_get_deploy_result, _ticket)
