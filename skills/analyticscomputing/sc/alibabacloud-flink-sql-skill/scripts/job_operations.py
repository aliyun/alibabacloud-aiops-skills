#!/usr/bin/env python3
"""SQL lifecycle implementation for Deployments, Jobs, State, and diagnostics."""
...
from git_sync import (
    api_result_data,
    api_result_error,
    api_result_succeeded,
    attach_git_sync,
    auto_delete_deployment,
    auto_sync_deployment,
    deployment_name,
    fetch_deployment,
    git_sync_enabled,
)
...
# ═══════════════════════════════════════════════════════════════════════════
# Deployment management (nine commands)
# ═══════════════════════════════════════════════════════════════════════════
...
def cmd_create_deployment(args):
    ...
    if api_result_succeeded(result):
        ...
        git_result = auto_sync_deployment(
            deployment if isinstance(deployment, dict) else {}, "create"
        )
        attach_git_sync(result, git_result)
...
def cmd_update_deployment(args):
    ...
    if api_result_succeeded(result):
        deployment = api_result_data(result)
        git_result = auto_sync_deployment(
            deployment if isinstance(deployment, dict) else {}, "update"
        )
        attach_git_sync(result, git_result)
    output(result, args.output)
...
def cmd_delete_deployment(args):
    ...
    chk = require_confirmation(
        "delete_deployment",
        f"Delete Deployment '{args.deployment_id}' permanently.",
        args.confirm,
    )
    if chk:
        return output(chk, args.output)
    ...
    git_prefetch_error = ""
    if git_sync_enabled():
        snapshot_result = fetch_deployment(args, args.deployment_id, client)
        if api_result_succeeded(snapshot_result):
            deleted_name = deployment_name(api_result_data(snapshot_result))
        else:
            git_prefetch_error = (
                "Failed to read the Deployment before deletion: " + api_result_error(snapshot_result)
            )
    ...
    if api_result_succeeded(result):
        git_result = auto_delete_deployment(deleted_name, git_prefetch_error)
        attach_git_sync(result, git_result)
    output(result, args.output)
...
def cmd_search_by_ip(args):
    err = require_args(
        args, "workspace", "namespace", "region_id", "ip", "dst_port"
    )
    if err:
        return output(err, args.output)
    ...
    req = GetDeploymentsByIpRequest(
        dst_ip=args.ip,
        dst_port=str(args.dst_port),
    )
...
def cmd_start_job(args):
    ...
    chk = require_confirmation(
        "start_job",
        f"Start a Job for Deployment '{args.deployment_id}'. This consumes compute resources.",
        args.confirm,
    )
    if chk:
        return output(chk, args.output)
...
def cmd_stop_job(args):
    ...
    chk = require_confirmation(
        "stop_job",
        f"Stop Job '{args.job_id}'. To avoid State loss, consider creating a Savepoint first.",
        args.confirm,
    )
    if chk:
        return output(chk, args.output)
...
def cmd_delete_job(args):
    ...
    chk = require_confirmation(
        "delete_job",
        f"Delete Job '{args.job_id}' permanently. Only a Job that is not running can be deleted.",
        args.confirm,
    )
    if chk:
        return output(chk, args.output)
...
def cmd_hot_update_job(args):
    ...
    chk = require_confirmation(
        "hot_update_job",
        f"HotUpdate running Job '{args.job_id}'. This modifies the live Job.",
        args.confirm,
    )
    if chk:
        return output(chk, args.output)
...
def cmd_create_savepoint(args):
    ...
    chk = require_confirmation(
        "create_savepoint",
        f"Create a Savepoint for Deployment '{args.deployment_id}'.",
        args.confirm,
    )
    if chk:
        return output(chk, args.output)
...
def cmd_delete_savepoint(args):
    ...
    chk = require_confirmation(
        "delete_savepoint",
        f"Delete Savepoint '{args.savepoint_id}' permanently.",
        args.confirm,
    )
    if chk:
        return output(chk, args.output)
...
# ═══════════════════════════════════════════════════════════════════════════
# Supporting operations (four commands)
# ═══════════════════════════════════════════════════════════════════════════
...
def cmd_get_lineage(args):
    ...
    # The backend requires these Boolean fields; their default value is False.
...
def cmd_get_checkpoints(args):
    """Return Flink Checkpoint statistics for a running SQL Job."""
    err = require_args(
        args, "workspace", "namespace", "region_id", "deployment_id", "job_id"
    )
    if err:
    ...
    ws, ns = _ws_ns(args)
    headers = FlinkApiProxyHeaders(workspace=ws)
    flink_job_id = args.job_id.replace("-", "")
    req = FlinkApiProxyRequest(
        flink_api_path=f"/jobs/{flink_job_id}/checkpoints",
        namespace=ns,
        resource_type="jobs",
...
def register(subparsers):
    """Register Deployment, Job, State, and diagnostic subcommands."""
    ...
    def _deployment_create_args(p):
        ...
        p.add_argument(
            "--engine_version", help="Engine version (for example, vvr-8.0.9-flink-1.17)"
        )
        p.add_argument(
            "--body_json", help="Complete Deployment request body JSON string (advanced)"
        )
        p.add_argument(
    ...
    def _deployment_update_args(p):
        p.add_argument("--deployment_id", required=True, help="Deployment ID")
        p.add_argument("--body_json", help="Updated Deployment request body JSON string")
        p.add_argument(
            "--confirm", action="store_true", help="Skip interactive confirmation"
        )

    def _page_args(p):
        p.add_argument("--page_size", help="Items per page")
        p.add_argument("--page_index", help="Page index")

    def _search_name(p):
        p.add_argument("--name", required=True, help="Deployment name to search for")
    ...
    def _search_ip(p):
        ...
        p.add_argument("--dst_port", type=int, required=True, help="Destination port")
    ...
    _add(
        "search_by_name", "Search for Deployments by name", cmd_search_by_name, _search_name
    )
    _add(
        "search_by_label",
        "Search for Deployments by Label",
        cmd_search_by_label,
        _search_label,
    )
    _add(
        "search_by_ip", "Search for Deployments by IP address", cmd_search_by_ip, _search_ip
    )

    def _events_args(p):
        p.add_argument("--deployment_id", required=True, help="Deployment ID")

    _add("get_events", "Get Deployment runtime events", cmd_get_events, _events_args)
    ...
    def _start_job_args(p):
        ...
        p.add_argument(
            "--restore_strategy",
            required=True,
            help="Restore strategy: NONE, LATEST_STATE, LATEST_SAVEPOINT, or FROM_SAVEPOINT",
        )
        p.add_argument("--body_json", help="Complete Start Job request body JSON (advanced)")
        p.add_argument(
            "--confirm", action="store_true", help="Skip interactive confirmation"
        )
    ...
    def _hot_update_result_args(p):
        p.add_argument(
            "--hot_update_id",
            required=True,
            help="HotUpdate ID returned by hot_update_job",
        )
    ...
    def _job_confirm(p):
        ...
        p.add_argument(
            "--stop_strategy",
            help="Stop strategy: NONE, STOP_WITH_SAVEPOINT, or STOP_WITH_DRAIN",
        )
        p.add_argument(
            "--confirm", action="store_true", help="Skip interactive confirmation"
        )
    ...
    _add(
        "delete_job", "Delete a Job instance that is not running", cmd_delete_job, _job_confirm
    )
    _add("hot_update_job", "HotUpdate a running Job", cmd_hot_update_job, _job_confirm)
    _add(
        "get_hot_update_result",
        "Get a HotUpdate result",
        cmd_get_hot_update_result,
        _hot_update_result_args,
    )
    _add("get_start_log", "Get the latest Job startup log", cmd_get_start_log, _job_args)
    _add("diagnose_job", "Diagnose a Job failure", cmd_diagnose_job, _job_args)
    ...
    _add(
        "delete_savepoint",
        "Delete a Savepoint permanently",
    ...
    def _resource_plan_args(p):
        p.add_argument("--deployment_id", required=True, help="Deployment ID")
        p.add_argument("--body_json", help="Flink Config JSON object used to generate the plan")

    def _ticket_args(p):
        p.add_argument("--ticket_id", required=True, help="Asynchronous ticket ID")
    ...
    def _flink_proxy_args(p):
        ...
        p.add_argument(
            "--resource_type",
            required=True,
            choices=("jobs", "sessionclusters"),
            help="Resource type (jobs or sessionclusters)",
        )
        p.add_argument(
            "--resource_id",
            required=True,
            help="Resource ID (Job ID for jobs; Session Cluster name for sessionclusters)",
        )

    def _lineage_args(p):
        p.add_argument(
            "--lineage_id", required=True, help="Lineage root-node ID (Job ID or Table ID)"
        )
        p.add_argument(
            "--lineage_id_type",
            required=True,
        ...
        p.add_argument(
            "--direction",
            default="BOTH",
            help="Lineage direction: UPSTREAM, DOWNSTREAM, or BOTH",
        )
        p.add_argument("--depth", type=int, default=1, help="Lineage depth (default: 1)")
        p.add_argument(
            "--is_column_level", action="store_true", help="Use column-level lineage"
        )
        p.add_argument(
            "--is_temporary", action="store_true", help="Mark the Table as temporary"
        )

    _add(
        "generate_resource_plan",
        "Generate a resource plan (asynchronous)",
        cmd_generate_resource_plan,
        _resource_plan_args,
    )
    _add(
        "get_resource_plan_result",
        "Get the resource-plan generation result",
    ...
    _add(
        "get_checkpoints",
        "Get completed, failed, and in-progress Checkpoint statistics",
        cmd_get_checkpoints,
        _job_args,
    )
    _add(
        "flink_api_proxy",
        "Proxy the Flink REST API (read-only)",
        cmd_flink_api_proxy,
        _flink_proxy_args,
    )
