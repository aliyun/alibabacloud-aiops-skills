...
## V6 — Verify specific MachineGroup detail (via list --machine-group-ids)

```bash
aliyun paistudio list-resource-group-machine-groups --region "${REGION_ID}" --resource-group-id "${RESOURCE_GROUP_ID}" --machine-group-ids "${MACHINE_GROUP_ID}"
```

**Pass when:** `TotalCount >= 1` and `MachineGroups[]` contains an entry whose `MachineGroupID` matches the requested ID, with fields `Cpu`, `Memory`, `Gpu`, `GpuType`, `EcsSpec`, `EcsCount`, `Status`, `GmtCreatedTime` present. If `TotalCount = 0`, the specified MachineGroup does not exist under this ResourceGroup.
