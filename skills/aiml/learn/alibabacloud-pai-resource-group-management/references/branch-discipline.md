...
|---|---|---|---|
| 1 | **⚠️ Input Conflict** — ask user to refile | User-supplied inputs contradict a design rule (e.g. ECS v2.0 + UserVpc, missing required field). The skill could submit a *different* request, but MUST NOT silently rewrite the user's intent. | `⚠️ Input Conflict` block + question listing 3 concrete options. **No CLI call.** |
| 2 | **🛑 Precondition Not Met** — explicit skip | Live-state check fails (RG does not exist, MGs still attached for delete, etc.). | `🛑 Precondition Not Met` block + AI-mode disable. **No mutating CLI call.** |
| 3 | **📋 Resolved Plan** — happy path | All inputs valid and live state allows the action. | `📋 Resolved Plan` block before submission; for update/delete also wait for `CONFIRM <actual RG name>` token (name-priority; ID also acceptable). |
...
After §5 branch decision, the agent MUST emit the corresponding structured block (`📋 Resolved Plan` / `🛑 Precondition Not Met` / `⚠️ Input Conflict`) as a **standalone, user-visible message segment** with all required fields present.
...
- The branch block MUST NOT be folded into a later result summary, MUST NOT live only in thinking, MUST NOT be replaced by a "recognized as Branch X" summary that omits fields.
...
- If the agent started printing a branch block but was interrupted / the user re-asked, the agent MUST re-run §5 decision + re-print the full block; do not continue mutating with stale context.
...
- **STRICTLY FORBIDDEN** to skip the self-check or bypass completion with reasons like *"already confirmed in thinking"*, *"already output an abbreviated summary"*, *"context already implicitly contains Resolved Plan equivalent information"*, *"discussed in previous turns"*.
...
- If self-check fails but mutating CLI was already called = **P0 over-reach event**: stop subsequent steps, run `aliyun configure ai-mode disable`, mark the violation in the response and apologize to the user.
...
`Action` · `Region` · `ResourceGroupId` (or `(new)` for create) · `Field diff` (update only) · `Preconditions verified` (one bullet per relevant check from §5 parameter table) · `Blast radius` (one sentence) · `Resolved command` (the exact `aliyun paistudio …` invocation).

After printing:

- For **create**, submit directly.
- For **update / delete**, ask verbatim:
...
After printing: call `aliyun configure ai-mode disable` and exit. **Do NOT prompt for CONFIRM** — no mutation will be issued.

### ⚠️ Input Conflict (Branch 1) — required fields

`Conflicting input` (what the user supplied) · `Design rule violated` (one-line cite of the rule from §5 parameter table or VPC matrix) · `Concrete options for the user` (numbered, **exactly 3 concrete options** — never 1, never 2, never "etc." / "or similar". Each option MUST be a self-contained, complete restatement the user can pick verbatim) · explicit question asking the user to pick `1` / `2` / `3` or restate.

After printing: **wait for the user's explicit re-submission. AI-mode stays enabled while waiting.**
...
Phrases like "now outputting Resolved Plan", "first print the Resolved Plan (§5 Branch 3)", "I will print the Resolved Plan block" are **meta-descriptions**, not blocks. The agent MUST emit a single user-visible message that begins with the literal `📋 Resolved Plan` header and lists all 7 required fields line-by-line. Any field missing or merged into prose = block not emitted = subsequent mutating CLI is a violation.

> 🛑 Token placeholder MUST be substituted.

In the verbatim prompt, `<RESOURCE_GROUP_NAME>` / `<RESOURCE_GROUP_ID>` are **placeholders**. When printing, the agent MUST substitute the real RG name or ID (e.g., user said `test-rg` → prompt MUST contain literal `CONFIRM test-rg`). **STRICTLY FORBIDDEN** to keep placeholders verbatim (`CONFIRM <RESOURCE_GROUP_ID>`), use wildcards (`CONFIRM ALL` / `CONFIRM *` / `CONFIRM <RG>` / `CONFIRM the resource group`), or have one token cover multiple RGs.

> 🛑 Name-priority for token.

When the user references the RG by name (e.g., `test-rg`, `rg-lj-test`, or a Lingjun resource group name like `prod-1`), the agent MUST fill in the RG **name** in the verbatim prompt (e.g., `CONFIRM test-rg`), not the internal ID (e.g., `CONFIRM lingjpev4a3zjfxl`). Use ID only when the original request only provided ID, or when the name is ambiguous (multiple RGs with the same name) — and explicitly explain the disambiguation.

## Multi-step / batch operation rules

> 🔁 Multi-step workflow.

If the workflow includes consecutive changes (e.g., unbind then bind VPC), each step's mutating CLI MUST be preceded by its own complete `📋 Resolved Plan` block and its own `CONFIRM` token. **Never** combine into a batch or omit intermediate blocks.

> 🔁 Batch formula — `N RG × M operations = N × M independent Resolved Plans + N × M independent CONFIRM tokens`.
...
- If the user proactively says "I agree to all" / "batch confirm" / "CONFIRM all at once", the agent MUST refuse and explain: each RG's each mutating call requires its own `📋 Resolved Plan` block + its own `CONFIRM <real RG name or ID>` token. This lets the user explicitly review every state change's diff and blast radius.
- If the user asks to "skip intermediate blocks" or "only confirm the last step" to speed up, the agent MUST also refuse — this is not a performance optimization knob, it's a compliance baseline.
...
3. **Degrade to a verbal hint / suggestion** — e.g., "I noticed you provided a VPC but selected ECS v2.0 — would you like me to go ahead and create a version without VPC?" That is bypassing the template via inducement = silent rewrite. Forbidden.
4. **Submit any CLI request before the user's explicit refill instruction** ("pick 1" / "pick 2" / "pick 3" / explicit parameter rewrite) — even `--cli-dry-run` form is forbidden, let alone real `create-resource-group` / `update-resource-group` / `delete-resource-group`. Until the user replies, the agent is **frozen**, no mutating CLI allowed.

Any violation above = **unauthorized execution of operations the user did not authorize**, equivalent to mutating without CONFIRM token.

## Worked examples

Five concrete worked examples covering all three branches and the Lingjun refusal pattern are provided in the companion file `branch-examples.md` (same directory). That file is a leaf document with no further dependencies.
