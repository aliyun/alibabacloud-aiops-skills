...
- ✅ Print resolved plan + `--cli-dry-run` output, then ask for `CONFIRM <RESOURCE_GROUP_ID>` before running update / delete.
- ❌ Running `delete-resource-group` directly after the user says "yes" / "go" / "okay".
...
- ✅ `list-resource-group-machine-groups` (with `--machine-group-ids` for specific MG lookup) remains allowed (read-only). This skill does NOT use `get-resource-group-machine-group` / `get-machine-group`.
- ❌ `aliyun paistudio get-resource-group-machine-group` — this skill uses `list-resource-group-machine-groups --machine-group-ids` instead.
- ❌ `aliyun paistudio get-machine-group` — same as above.
