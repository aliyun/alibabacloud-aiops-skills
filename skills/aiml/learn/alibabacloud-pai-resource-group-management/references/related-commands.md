...
| --- | --- | --- | --- |
| `aliyun paistudio list-resource-group-machine-groups` | List MGs in an RG / get specific MG detail | `--region`, `--resource-group-id` | filters: `--machine-group-ids` (specific MG lookup), `--name`, `--creator-id`, `--ecs-spec`, `--payment-type`, `--status`, `--order-instance-id`, `--disk-pl`, `--payment-duration`, `--payment-duration-unit`, `--sort-by`, `--order` |

> ⚠️ **This skill does NOT use `get-resource-group-machine-group` / `get-machine-group`.** To view a specific MG's details, use `list-resource-group-machine-groups --machine-group-ids "${MACHINE_GROUP_ID}"` exclusively.
...
Rationale: MachineGroup release is billing-impacting and triggers refund / repurchase semantics that the agent cannot reason about safely. See `not-implementable.md`.
