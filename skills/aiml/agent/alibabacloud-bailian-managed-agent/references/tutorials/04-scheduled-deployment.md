...
def collect_runs(deployment_id, processed_run_ids):
    """Use run_id as the idempotency key so one run is never processed twice."""
...
**`run_id` must be persisted as the idempotency key.** Worker restarts, overlapping pulls, and
