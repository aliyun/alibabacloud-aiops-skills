...
| Multi-agent formation + approval | multiagent.coordinator / SessionThread / main-Agent approvals | Validate each member, create the coordinator, then separate output by `thread_id` | Current official docs limit tool approvals to the main Agent; `session_thread_id` does not enable member approvals |
