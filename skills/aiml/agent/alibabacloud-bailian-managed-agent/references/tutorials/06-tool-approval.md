...
> The policy is now documented in the [official tool guide](https://docs.agent.bailian.aliyun.com/zh/managed-agents/build-agent/tools.md). Approval currently applies to the main Agent, not sub-agent tools. The form below has also been tested against the real server.
>
...
Add `permission_policy` to high-risk tools in the Agent's tool config (documented object form, also field-tested):
...
def run_with_approval(session_id, approve_fn, max_rounds=10):
    """Resume a Session already waiting for approval.

    approve_fn(call_details) -> ("allow" | "deny", deny_message or None).
    The application authenticates the human and displays the exact tool/arguments.
    Initial task submission and a total business deadline belong to the caller.
    """
    for _ in range(max_rounds):
        info = client.sessions.retrieve(session_id)
        reason = info.stop_reason
        reason = reason.to_dict() if hasattr(reason, "to_dict") else (reason or {})
        if reason.get("type") != "requires_action":
            assert_success(reason.get("type"), session_id)
            return reason
        batch_id, call_ids = reason.get("pending_batch_id"), reason.get("pending_call_ids") or []
        if not batch_id or not call_ids:
            raise RuntimeError("Pending state is not a tool approval; inspect it manually")
        # Read all pages; an older approval or another batch is not authorization for this one.
        calls, cursor, seen = {}, None, set()
        while True:
            page = client.sessions.events.list(session_id, types=["tool_approval_request"],
                                               order="desc", limit=100, page=cursor)
            for ev in page.data:
                for block in ev.content or []:
                    data = getattr(block, "data", None)
                    if isinstance(data, dict) and data.get("batch_id") == batch_id and data.get("call_id") in call_ids:
                        calls[data["call_id"]] = data
            cursor = page.next_page
            if len(calls) == len(call_ids) or not cursor:
                break
            if cursor in seen:
                raise RuntimeError("repeated approval page cursor")
            seen.add(cursor)
        if set(calls) != set(call_ids):
            raise RuntimeError("Missing exact pending tool details; do not approve blindly")
        responses = []
        for call_id in call_ids:
            result, deny_message = approve_fn(calls[call_id])
            if result not in ("allow", "deny"):
                raise ValueError("approval must be allow or deny")
            responses.append(tool_approval_response(batch_id, call_id, result, deny_message))
        # Subscribe BEFORE submitting the batch. A post-then-stream sequence can miss completion.
        with client.sessions.events.stream(session_id, timeout=600.0) as stream:
            client.sessions.events.send(session_id, responses)
            for ev in stream:
                reason_type = handle_event(ev)  # returns a string, not the full stop_reason
                if reason_type:
                    break
            else:
                raise RuntimeError("Stream ended without a terminal event; reconcile history")
        if reason_type != "requires_action":
            assert_success(reason_type, session_id)
            return {"type": reason_type}
    raise RuntimeError("Approval round-trip limit exceeded; preserve Session for recovery")
...
- **MultiAgent scope**: the current official contract supports tool approval for the main Agent only, not sub-agent tools. Do not promise member-level approval just by adding `session_thread_id`. Thread targeting for messages/interrupts is a separate feature; see [07](07-multiagent-coordinator.md).
