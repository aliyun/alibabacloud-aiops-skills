...
the one with the `sthr_` prefix), selecting which thread they act on. **Approvals and interrupts in fleet scenarios must be targeted**,
otherwise they may land on the wrong thread:
...
Targeted approval responses are likewise hand-built per tutorial 06 (the SDK's `user_tool_confirmation` is unusable),
adding `session_thread_id` at the wire top level:

```python
resp = {"role": "user", "type": "tool_approval_response",
        "session_thread_id": thread_id,          # ← the targeting field, at the event top level
        "content": [{"type": "data",
                     "data": {"batch_id": batch_id, "call_id": call_id, "result": "allow"}}]}
client.sessions.events.send(session_id, [resp])
```

Take `thread_id` from the event that raised the approval (top level of `tool_approval_request`). Mind the direction difference:
**sending** uses `session_thread_id`; the server's **echo / event stream** normalizes it to `thread_id`.
The full approval round trip is in [06-tool-approval.md](06-tool-approval.md) — `result` still accepts only `allow` / `deny`.

### 7. Editing the fleet and reverting to single-agent

- Add/remove members → `POST /agents/{id}` updating `multiagent.agents`, **must carry the current `version` as the optimistic lock**
...
- [ ] Members with write operations have approvals configured (`permission_policy`), and targeted approvals carry `session_thread_id` (send direction)
