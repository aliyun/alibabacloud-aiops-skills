...
For scratch trials, run `init` in a temporary working directory: CLI 1.25.0 also updates the current directory’s `.gitignore`, even when `--file` points elsewhere. See [provisioning side effects](../workflows/provision.md).
...
- Built-in `web_search` / `web_fetch` are launched and preferred for search/page reading. Enable these exact names and read back the Agent config. `browser_use` is available through MCP.
  A successful `validate` alone does not establish runtime tool availability.
...
| A configured tool is unusable by the Agent | The tool name is unsupported or not enabled; supported tools include the shell/file tools and `web_search` / `web_fetch`; `download_file` is decommissioned. Local validation alone does not prove runtime availability | Configure supported tools, including `web_search` / `web_fetch` for web tasks; `mark_artifacts` works unconfigured; after creation, re-check the agent's `tools` |
