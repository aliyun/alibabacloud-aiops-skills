...
client = Client(
    ...
    region="cn-beijing",
    max_retries=0,  # mutations must be reconciled before retry
)
```
...
**Prerequisite readiness gates**: an Environment with preinstalled packages may still be preparing after creation, and Session creation can fail during that window. Wait using the [environment readiness guidance](../tutorials/05-environment-packages.md). Uploaded files must pass detection/security review and reach `available` before attachment; use the bounded [file polling loop](../tutorials/03-mount-files.md). The abbreviated snippet below assumes both gates have passed.

```
(1) Upload a file (optional), wait for available; wait for Environment preparation
→ (2) Create a Session (bind Agent + Environment, attach ready files, attach Vault)
...
# (3) Subscribe first: even a long task can fail immediately. SSE has no history replay.
with client.sessions.events.stream(session.id, timeout=600.0) as stream:
    ...
    for ev in stream:
        if ev.type == "message" and ev.role == "assistant":
            ...
            if ev.session_status in ("idle", "terminated"):
                ...
                if reason or ev.session_status == "terminated":
                    break
...
After the loop, require `reason == "end_turn"`; treat `requires_action` as pending and other endings as failures. See `assert_success` in the full implementation.
...
## Launched extensions: images, Delta and Thinking

- Images: use `content: [{"type":"image","file_id":"<available image>"}]` alongside a text block. Inline Base64 `image_data` is currently unsupported (user-confirmed 2026-09-18); the earlier official documentation claiming support was incorrect. Use the tested file-ID path. Consult updated docs and re-test before adopting any future Base64 support; see [cookbook 10](../cookbook/10-multimodal-delta-and-thinking.md).
- Delta: repeated `event_deltas[]=message` and `event_deltas[]=reasoning` on the SSE GET enable `event_start` / `event_delta` plus final full messages. Group by event ID and content-block index; do not append the final full message twice. SDK 1.27.3 has no corresponding stream argument; use REST.
- Thinking: `reasoning` start/final events may occur without text deltas. Do not require visible reasoning text to recognize progress.
- Error details are in the top-level `error` object; tool failure is the top-level `is_error` flag.
