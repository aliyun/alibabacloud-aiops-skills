...
| | `GET /agents` | List |
| | `POST /agents/{agent_id}` | Update (**partial replace**: passed fields replace, omitted fields unchanged; auto-generates a new version number each time; must carry the existing `name` and current `version` as an optimistic lock) |
...
| **Session resources** | `POST /sessions/{session_id}/resources` | Runtime dynamic file mounting (no restart; verify sandbox visibility, not just HTTP success; Memory Store excluded) |
...
| **Memory Store** | `POST /memory_stores`, `GET /memory_stores[/{id}]`, `POST /memory_stores/{id}`, `POST /memory_stores/{id}/archive` | MA working-memory container; distinct from UserId-scoped Bailian Memory Library |
| **Memory** | `POST /memory_stores/{id}/memories`, `GET …/memories[/{memory_id}]`, `POST …/memories/{memory_id}`, `DELETE …/memories/{memory_id}` | UTF-8 working-note files; update can use `precondition.content_sha256` |
| **Memory Version** | `GET /memory_stores/{id}/memory_versions[/{version_id}]` | Created/modified/deleted history; see cookbook 09 |
...
| `config.packages` | No | Yes | Declared grouped by package manager; **installation is asynchronous**; creation success does not imply readiness, and Session creation may fail while the Environment is preparing. Earlier probes observed ~2 minutes and no status field; use current documented readiness signals or bounded retries of explicit not-ready rejections, not a guaranteed fixed wait (see [readiness guidance](../tutorials/05-environment-packages.md)). Field-tested groups: `apt` / `pip` / `npm` / `gem` install; **`cargo` / `go` declarations install nothing (silently ineffective, no error)**; the official docs list only apt / pip / npm. Updates are **whole-list replaces** (unlisted packages are removed) with a **minutes-level propagation delay** (field test: a new session 13 seconds later still saw the old environment; effective after 184 seconds)|
