    ...
    public static String handle(JsonNode ev, StringBuilder textBuffer) {
        ...
        if ("message".equals(type) && "assistant".equals(ev.path("role").asText())
            ...
            log.error("server error event: code={} message={}",
                    ev.path("error").path("code").asText(null), ev.path("error").path("message").asText(null));
            ...
            if (status != null && TERMINAL_STATUSES.contains(status)) {
                ...
                return reason != null ? reason : ("terminated".equals(status) ? "terminated" : null);
            }
        } else {
...
/** Follow every page; only completed runs may be marked as processed. */
public void collectRuns(String deploymentId, Set<String> processedRunIds) throws IOException {
    ...
    while (true) {
        String path = "/deployments/" + deploymentId + "/runs?limit=50";
        if (cursor != null) {
            path += "&page=" + java.net.URLEncoder.encode(cursor, "UTF-8");
        }
        JsonNode page = client.get(path);
        for (JsonNode run : page.path("data")) {
            String runId = run.path("id").asText();
            if (processedRunIds.contains(runId)) continue;
            // Single-run query uses the global path; the nested path returns 404.
            JsonNode detail = client.get("/deployment_runs/" + runId);
            String status = detail.path("status").asText();
            if (!"succeeded".equals(status) && !"failed".equals(status)) continue;
            handleRunResult(detail);
            processedRunIds.add(runId);
            // Production: atomically commit result + durable dedup marker.
            // External delivery needs an outbox and receiver-side idempotency.
        }
        cursor = page.path("next_page").asText(null);
        if (cursor == null || cursor.isEmpty()) break;
        if (!seenCursors.add(cursor)) throw new IOException("repeated run page cursor");
    }
}
```
