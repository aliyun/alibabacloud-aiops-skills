...
client = Client(
    ...
    max_retries=0,                                   # avoid duplicate writes on ambiguous timeouts
)
```
...
**Mutation retry policy**: SDK 1.27.3 retries POST on timeout when `max_retries>0`. Keep automatic retries disabled for this shared client. Retry reads with bounded backoff; reconcile Session creation and event-send outcomes before any write retry. A business idempotency key in metadata is correlation, not server-enforced deduplication.
...
def handle_event(ev, on_text=None, on_tool=None, on_member_text=None):
    ...
    if ev_type == "message" and getattr(ev, "role", None) == "assistant":
        for block in ev.content or []:
            if getattr(block, "type", None) == "text":
                ...
                if thread_key.startswith("sthr_"):
                    if on_member_text:
                        on_member_text(thread_key, text)
                elif on_text:
                    on_text(text)
        ...
        raw = ev.to_dict() if hasattr(ev, "to_dict") else vars(ev)
        ...
        if ev.session_status in TERMINAL_STATUSES:
            ...
            return reason.get("type") or ("terminated" if ev.session_status == "terminated" else None)
...
pending = info.stop_reason.to_dict() if hasattr(info.stop_reason, "to_dict") else info.stop_reason
# Only respond after the caller has approved the exact pending tool and arguments.
with client.sessions.events.stream(session_id, timeout=600.0) as stream:
    client.sessions.events.send(session_id, [
        tool_approval_response(batch_id=pending["pending_batch_id"],
                               call_id=call_id, result="allow")
        for call_id in pending["pending_call_ids"]
    ])
    for ev in stream:
        reason_type = handle_event(ev)
        if reason_type:
            assert_success(reason_type, session_id)
            break
# Repeated approvals, current-batch filtering and total deadlines: tutorial 06.
```
...
def poll_task(biz_order_no, task_repo, max_wait=3600):
    ...
    while time.monotonic() < deadline:
        ...
        if info.status in ("idle", "terminated"):
            ...
            stop_reason = (reason or {}).get("type")
            if stop_reason:
                break
...
# a mismatch conflicts (409). Also preserve name: omitting it returned 400 AGENT_005
# in the 2026-09-18 test. Other omitted fields stay unchanged.
current = client.agents.retrieve(AGENT_ID)
client.agents.update(AGENT_ID, name=current.name, version=current.version, system_prompt="...")
