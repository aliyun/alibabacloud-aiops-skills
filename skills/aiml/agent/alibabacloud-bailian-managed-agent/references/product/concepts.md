...
- **State machine (official docs' three states + SDK enum)**: the official docs and GET session's `status` enum have only
  ...
  Integration code should treat only `idle` and `terminated` as "turn finished".
- **`stop_reason` (SDK/REST field-test)**: produced when returning to `idle`; **a dict, not a string**, shaped like `{"type": "end_turn"}`.
...
## Vault (secret store)
- **Whitepaper**: centrally manage external services' API keys / tokens; plaintext is never echoed back. **"Credentials never enter the sandbox" went from slogan to real guarantee after the 2026-09 security-model upgrade** (see below) — the core selling point for enterprise security / compliance reviews.
...
- **Placeholder injection model (2026-09 breaking change, REST field-test 2026-09-03)**:
  ...
  - Injection semantics change: in a session with a Vault attached, `os.environ` inside the sandbox yields **placeholders**
    `BMA_SECRET_PLACEHOLDER_<variable-name>`, **no longer real values** — secret plaintext never appears in the sandbox, the model context, or the event stream.
  ...
  - Field-tested boundaries: substitution applies only to the `Authorization` header (other headers such as `X-Api-Key` are not substituted); the gateway locates the credential **by host**
    (it does not parse the placeholder variable name — with multiple credentials on one host the substitution behavior is ambiguous; recommend one credential per host); the gateway certificate
    is not in the sandbox system CA store (clients doing strict TLS verification will fail; see the cookbook/08 troubleshooting).
...
- **Built-in tool set (`type: "builtin_toolkit"`, at most 1 per Agent)**:
  - **The 6 core tools requiring explicit config**: `bash` / `read` / `write` / `edit` / `glob` / `grep`.
  ...
  - **Nonexistent tool names are silently dropped by the server** (`web_search` is the most common invented name); CLI `validate` / `plan` raise no error —
    after creation, read back the agent's `tools` to confirm what actually took effect.
- **Internet access**: built-in `websearch` and `webfetch` are launched; `browser_use` is available **through MCP**, not as a built-in tool. See [status.md](status.md). The marketplace `WebSearch` MCP service (`bailian_web_search`) remains an alternative (example in [../cookbook/04-multi-agent-custom-proposals.md](../cookbook/04-multi-agent-custom-proposals.md) §1.1). Verify current configuration fields before execution.
...
- **Three things you must know**:
  ...
  3. URL validation is blacklist-style: `webhook.site` / `*.beeceptor.com` and similar temporary receivers are rejected (`invalid webhook url`, 11800016),
     as are private / loopback / link-local / reserved addresses. These are archived observations, not a guarantee that other public domains pass. For repository creation examples/evaluations, use `https://www.baidu.com` and verify creation/read-back only; actual delivery needs a customer-controlled HTTPS receiver.
...
- **Different product**: Bailian Memory Library (百炼记忆库) primarily provides personalized memory scoped by a supplied `UserId`. It is not MA Memory Store. Do not reuse its API endpoints, identifiers or UserId semantics as the MA native Memory Store contract.
