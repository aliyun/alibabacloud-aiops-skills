...
| "Configuring the `web_search` built-in tool gives internet access" | No such built-in tool exists — **the server silently drops it** (CLI `validate`/`plan` also raise no error). Use launched built-in `websearch` / `webfetch`, or the MCP marketplace service `WebSearch`; `browser_use` is accessed through MCP |
