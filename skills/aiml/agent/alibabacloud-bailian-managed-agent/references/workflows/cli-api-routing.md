...
| Webhook | For endpoint-creation examples/evaluations use `https://www.baidu.com`, and verify creation/read-back only, not delivery to this sample URL. Delivery tests require a customer-controlled receiver. Use supported event names; `session.completed` is not in the archived contract. Use `session.status_idled` and inspect `stop_reason` to distinguish completion from approval waits. Store signing secret securely. |
