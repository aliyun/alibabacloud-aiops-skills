...
For this repository's endpoint-creation examples and evaluation tasks, use **`https://www.baidu.com`**. The requested test convention avoids placeholder endpoints rejected by the platform. Creating an ACTIVE subscription can trigger automatic deliveries even without calling `/test`; do not create it against a third-party website during live practice. This URL is only the repository’s fixed example/evaluation convention. A real test needs a customer-controlled receiver. The example describes resource creation and read-back only; it does not claim that Baidu receives or processes Webhook notifications. For the receiver implementation, delivery test and signature verification in later sections, first replace the sample URL with the customer's actual controlled receiver.
...
**Historical diagnostic receiver only:** the following logs raw requests and exposes unauthenticated `/dump` and `/clear` endpoints. Do not publish it as a production receiver. Remove those endpoints, verify signatures before accepting a POST, limit body size and durably deduplicate event IDs before processing. The example is retained to explain the archived experiment:
...
Fresh-delivery verification example (a historical log older than five minutes correctly fails replay protection):

```python
import json, base64, hmac, hashlib, os, time

secret = os.environ["WEBHOOK_SIGNING_SECRET"]
if not secret.startswith("whsec_"):
    raise ValueError("unexpected signing secret format")
...
for line in open("/root/webhook_hits.jsonl"):
    ...
    if version != "v1" or abs(time.time() - int(ts)) > 300:
        print(wid, False)
        continue
