...
**Builtin tools**: six configurable core tools — `bash`, `read`, `write`, `edit`, `glob`, `grep`, **each requiring `enabled: true` in `configs[]`**. The platform additionally auto-injects `mark_artifacts` (no declaration needed). `download_file` is officially offline — stop configuring it. No `web_search` / `web_fetch`; going online means MCP.
