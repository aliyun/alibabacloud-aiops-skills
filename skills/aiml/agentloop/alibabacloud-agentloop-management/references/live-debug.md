...
> Global conventions (credentials, Observability / User-Agent, output format, error codes, command prefix) - see [live-debug-runtime.md](live-debug-runtime.md).
> RAM permissions - see the `live-debug-ram-policies.md` section listed in [live-debug-runtime.md](live-debug-runtime.md#credentials).
...
- `LIVE_DEBUG_SESSION_ID` - the session-id for the current workflow (32 lowercase hexadecimal characters; generation rules in [live-debug-runtime.md](live-debug-runtime.md#session-id-rule)). When set, the scripts first require the version from `references/manifest.json`, then append `--user-agent "AlibabaCloud-Agent-Skills/alibabacloud-agentloop-management/skill-version/{version}/{session-id}"` to all `aliyun cms2` / `aliyun sls` calls.
