...
Managing the experience store itself (see `references/experience/context-store-management.md`) uses the aliyun CLI identity and requires the corresponding AgentLoop actions: `CreateContextStore`, `ListContextStores`, `GetContextStore`, `UpdateContextStore`, `DeleteContextStore`, and the `*ContextStoreApiKey*` actions for API Key management.

## Data Flow

When recall is enabled, `scripts/experience/search_context.py` sends the following data to the configured AgentLoop SearchContext endpoint:
