...
When recall is enabled, `scripts/search_context.js` sends the following data to the configured AgentLoop Recall or SearchContext endpoint:

- `query`: the task, error, case, service, or goal text provided by the agent
- AK/SK mode: `context_type` (`experience` or `memory`), `limit`, `threshold`, `filter`, and `formatted`
