...
On `Unauthorized`/`AccessDenied`: stop the affected operation, report `[Error: permission]`, the exact missing Action, and request ID, then pause for permission diagnosis. Ask once in that turn and end the turn. Retry only after the user explicitly confirms authorization is complete. If the user says authorization is incomplete/rejected/cancelled, or the ask limit is reached, emit `[BLOCKED: PERMISSION_REQUIRED] Authorization was not confirmed. The affected operation and all dependent cloud calls are halted.` Never switch account/profile or widen scope.
