...
**Scenario**: LoongCollector install + cloud-side ops (onboarding, config, machine group, Lens, basic troubleshooting)
**Purpose**: install stage gate + unified acceptance (U1-U6) + CLI command correctness patterns

## 0. Install stage gate (not full-loop success)

| Environment | Pass | Then |
|---|---|---|
| ECS / self-host | `loongcollectord status` → `loongcollector is running` | onboarding.cloud + U1-U6 |
| ACK | addon `loongcollector` active; `loongcollector-ds` Ready via CS addon resources | reuse `k8s-log-${cid}` / `k8s-group-${cid}`; **SLS API default**; U1-U6 |
| Self-k8s | `loongcollector-ds` Running; controller/operator present | reuse values Project / official group; CRD or API; U1-U6 |

Process/Addon ready alone is not collection success. U5 empty → not verifiable, never report success.
...
For U5, the final user-facing answer must contain one exact data-state token and the exact reason token defined by the Verify contract in `SKILL.md`.
...
| U3 | Applied state | API read succeeds and recent update readable; CRD write path: `status.success=true` + `lastAppliedConfig` non-empty | check management plane, permission, double-write, controller |
...
| U5 | Data arrival | new data in the acceptance window; if no new source logs, mark "not verifiable". Include the exact data-state and reason tokens from `SKILL.md`. | check input path, processing, send, source logs |
...
- CORRECT: every cloud-API command has `--user-agent "AlibabaCloud-Agent-Skills/alibabacloud-loongcollector-ops session/<session-id>"` and `--region <r>`
...
- After `get-logging` fails to yield a usable Lens entry, the turn ends with the fixed Lens-entry fallback subject from `SKILL.md` plus `[AWAITING: LENS_ENTRY]`. Do not write the troubleshooting conclusion in that same turn. `get-applied-configs` must already have been attempted once even if the business Project 404s. After the user supplies the entry, run `logtail_alarm` and the version-routed metric query, then conclude.
- Troubleshooting always emits every applicable field-complete Heartbeat, Alarm, Collection, Binding, and Pipeline evidence line from `SKILL.md` §10 in the **user-facing final answer**, using `N/A` plus attempted command/error when a resource is missing, and `resource_status: Resource not found` for missing resources. A later explicit request to end the task must not drop those tokens.
