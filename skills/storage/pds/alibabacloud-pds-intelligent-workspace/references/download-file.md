...
**Scenario**: Download a file from PDS to local — **only** when the user explicitly wants the raw file saved locally.

> If the intent is to **analyze / summarize / close-read** a document, audio, or video, do **NOT** download it here and read it yourself. Use `aliyun pds analyze` — see `references/multianalysis-file.md`.
...
Returns `{ "drive_id": ..., "file_id": ..., "remote_path": ..., "local_path": ..., "size": <bytes> }`. The size is verified against the server; a mismatch fails the command.
